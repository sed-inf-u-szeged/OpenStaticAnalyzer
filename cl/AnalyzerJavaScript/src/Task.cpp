...
Task::ExecutionResult RunESLintTask::execute()
{
  ...
  try {
    ...
    if (!props.profileXML.empty()) {
      sv.push_back("--rul");
      const string rul = (props.tempDir / "TSLint.rul.md").string();
      sv.push_back(rul);
    }
    ...
    //Add temp dir
    sv.push_back("--tempDir");
    sv.push_back(props.tempDir.string());

    //Add execution dir
    sv.push_back("--executionDir");
    sv.push_back((props.toolsDir / "node_modules" / "eslintrunner").string());
...
Task::ExecutionResult ESLint2GraphTask::execute()
{
  ...
  if (props.runESLint) {
    try {
      ...
      sv.push_back("-rul:" + (props.tempDir / "TSLint.rul.md").string());
...
Task::ExecutionResult ProfileTask::execute()
{
  ...
  try {
    ...
    // original rul files
    path ESLintRulFileOrig = props.toolsDir / "TSLint.rul.md";
    path ESLintRulMetadataFileOrig = props.toolsDir / "TSLint.rul_metadata.md";

    // temp rul files
    path ESLintRulFile = props.tempDir / "TSLint.rul.md";
    path ESLintRulMetadataFile = props.tempDir / "TSLint.rul_metadata.md";
    ...
    if (exists(ESLintRulMetadataFileOrig)) {
      copy_file(ESLintRulMetadataFileOrig, ESLintRulMetadataFile);
    }
    ...
    //process udm metrics
    bool UDM_result = profileProcessUDM(profile, (props.tempDir / "UDM.rul.md").string(), "javascript");
