...
Task::ExecutionResult Pylint2GraphTask::execute()
{
  ...
  try {
    ...
    sv.push_back("-rul:" + (props.tempDir / "Pylint.rul.md").string());
...
Task::ExecutionResult ProfileTask::execute()
{
  ...
  try {
    ...
    // original rul files
    path PylintRulFileOrig = props.toolsDir / (props.pythonVersion == "2" ? "Pylint_1.rul.md" : "Pylint_2.rul.md");
    const auto PylintRulMetadataFilename = (props.pythonVersion == "2" ? "Pylint_1.rul_metadata.md" : "Pylint_2.rul_metadata.md");
    path PylintRulMetadataFileOrig = props.toolsDir / PylintRulMetadataFilename;

    // temp rul files
    path PylintRulFile = props.tempDir / "Pylint.rul.md";
    path PylintRulMetadataFile = props.tempDir / PylintRulMetadataFilename;

    if (exists(PylintRulFileOrig))
      copy_file(PylintRulFileOrig, PylintRulFile);
    if (exists(PylintRulMetadataFileOrig))
      copy_file(PylintRulMetadataFileOrig, PylintRulMetadataFile);
    ...
    //process udm metrics
    bool UDM_result = profileProcessUDM(profile, (props.tempDir / "UDM.rul.md").string(), "python");
