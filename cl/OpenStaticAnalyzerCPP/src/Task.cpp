...
Task::ExecutionResult ProfileTask::execute()
{
  ...
  try {
    ...
    // original rul files
    path CppcheckRulFileOrig = props.toolsDir / "Cppcheck.rul";
    path ClangTidyRulFileOrig = props.toolsDir / "ClangTidy.rul";

    // temp rul files
    path CppcheckRulFile = props.tempDir / "Cppcheck.rul";
    path ClangTidyRulFile = props.tempDir / "ClangTidy.rul";
    ...
    //process udm metrics
    bool UDM_result = profileProcessUDM(profile, (props.tempDir / "UDM.rul").string(), "cpp");
...
Task::ExecutionResult Cppcheck2GraphTask::execute()
{
  ...
  try {
    ...
    sv.push_back("-rul:" + (props.tempDir / "Cppcheck.rul").string());
...
Task::ExecutionResult ClangTidyTask::execute()
{
  ...
  try {
    ...
    if (loadStringListFromFile((wrapperWorkDirLog / "ast.list").string(), astList))
    {
      ...
      tidy2graphArgs.push_back("-rul:" + (props.tempDir / "ClangTidy.rul").string());
...
namespace {
  ...
  { 
    ...
    //Get group ids.
    rulHandler.getGroupIdList(groupIDs);

    for(const auto& groupID : groupIDs)
    {
      rulHandler.getGroupMembers(groupID, groupMembers);

      if (!rulHandler.getIsEnabled(groupID))
      {
        disabledRules.push_back(groupID + "*");
      }
      else
      {
        for (const auto& rule : groupMembers)
        {
          if (!rulHandler.getIsEnabled(rule))
          {
            disabledRules.push_back(rulHandler.getOriginalIdByRuleId(rule));
          }
        }
      }
      groupMembers.clear();
