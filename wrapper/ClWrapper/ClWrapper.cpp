...
#define CL_WRAPPER  "ClWrapper"

namespace ColumbusWrappers {

  ClWrapper::ClWrapper(string configfile) : AbstractWrapper(configfile), AbstractCompiler(configfile), AbstractLinker(configfile), compWarnings(), compArgs(), linkArgs(), prepArgs() {
    mode = wrapper_cl;
  }

  bool ClWrapper::process(int argc, char** argv) {
    ...
    if (ClWrapperSup::compilerArguments(temp_args, compWarnings, prepArgs, compArgs, linkArgs)) {

      for (list<string>::iterator it_w = compWarnings.can_warning.begin(); it_w != compWarnings.can_warning.end(); it_w++) {
        writeWarningMsg(CL_WRAPPER, CMSG_WRAPPER_TOOL_WARNINGS, it_w->c_str());
      }

      for (list<string>::iterator it_u = compWarnings.unrec_args.begin(); it_u != compWarnings.unrec_args.end(); it_u++) {
        writeWarningMsg(CL_WRAPPER, CMSG_WRAPPER_TOOL_UNRECOGNIZE_ARGUMENTS, it_u->c_str());
      }
