...
namespace ColumbusWrappers {
  class ClWrapper : public AbstractCompiler, public AbstractLinker {
/**
 * @brief This class can processing parameter list of cl.exe and calling CAN and CANLink.
 */
  public:
    /**
     * @brief Public constructor for ClWrapper class.
     *
     * @param configfile     [in] The input configuration file (analyzer_wrapper_config.ini) for analyzer wrapper.
     */
    ClWrapper(std::string configfile);

    /**
     * @brief It calls parameter processor functions and calls CAN and CANLink.
     *
  ...
  private:
    Warnings compWarnings;        ///< The problematic and unknow arguments are put into this struct.
