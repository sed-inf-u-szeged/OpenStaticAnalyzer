...
#define LINK_WRAPPER    "LinkWrapper"

namespace ColumbusWrappers {

  LinkWrapper::LinkWrapper(string configfile) : AbstractWrapper(configfile), AbstractLinker(configfile), linkWarnings(), linkArgs() {
    mode = wrapper_cl;
  }

  bool LinkWrapper::process(int argc, char** argv) {
    ...
    if (instrument_enable) {
      ...
      cmd += " Ws2_32.lib";
      cmd = wrappedExe + cmd;
      writeInfoMsg(LINK_WRAPPER, CMSG_ANALYZER_WRAPPER_COMMAND, cmd.c_str());
      bool linker_run = systemCall(cmd);
      return linker_run;

    } else {

      if (ClWrapperSup::linkerArguments(temp_args, linkWarnings, linkArgs)) {

        for (list<string>::iterator it_w = linkWarnings.can_warning.begin(); it_w != linkWarnings.can_warning.end(); it_w++) {
          writeWarningMsg(LINK_WRAPPER, CMSG_WRAPPER_TOOL_WARNINGS, it_w->c_str());
        }

        for (list<string>::iterator it_u = linkWarnings.unrec_args.begin(); it_u != linkWarnings.unrec_args.end(); it_u++) {
          writeWarningMsg(LINK_WRAPPER, CMSG_WRAPPER_TOOL_UNRECOGNIZE_ARGUMENTS, it_u->c_str());
        }
