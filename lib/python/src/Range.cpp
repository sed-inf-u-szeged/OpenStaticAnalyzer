...
namespace columbus { namespace python { namespace asg {
  ...
  Range::Range(StrTable &_strTable, const std::string& pathString, unsigned line, unsigned col, unsigned endLine, unsigned endCol) : strTable(&_strTable), positionInfo() {
  ...
  void Range::setPath(const std::string& s) {
    if (!strTable)
      throw PythonException(COLUMBUS_LOCATION,CMSG_EX_UNTIL_NO_STRTABLE_IS_SET_THE_SETPATH );
  ...
  Range::Range(StrTable &_strTable, Key pathKey, unsigned line, unsigned col, unsigned endLine, unsigned endCol) : strTable(&_strTable), positionInfo() {
  ...
  void RangeListIterator::add(const Range &r) {
    switch (lastOp) {
      ...
      case op_Add:
        ...
        break;
      case op_Invalidated:
        throw PythonInvalidIteratorException(COLUMBUS_LOCATION, CMSG_EX_THE_ITERATOR_IS_INVALID);
      case op_Previous:
      case op_Remove:
        break;
    }
  ...
  void RangeListIterator::remove() {
    switch (lastOp) {
      ...
      case op_Add:
      case op_Remove:
        throw PythonIllegalStateException(COLUMBUS_LOCATION, CMSG_EX_NEITHER_NEXT_NOR_PREVIOUS_HAVE_BEEN_CALLED);
      case op_Next:
      case op_Previous:
        it = safeRemove(it);
        break;
  ...
  const Range& RangeListIterator::next() {
    ...
    if (it == container->end())
        throw PythonNoSuchElementException(COLUMBUS_LOCATION, CMSG_EX_THE_ITERATION_HAS_NOT_NEXT_ELEMENT);
  ...
  const Range& RangeListIterator::previous() {
    ...
    if (it == container->end())
        throw PythonNoSuchElementException(COLUMBUS_LOCATION, CMSG_EX_THE_ITERATION_HAS_NOT_PREVIOUS_ELEMENT);
