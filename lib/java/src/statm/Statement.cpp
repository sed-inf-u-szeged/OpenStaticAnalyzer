...
namespace statm { 
  ...
  void Statement::clone(const Statement& other, bool tryOnVirtualParent) {
  ...
  double Statement::getSimilarity(const base::Base& base){
    if(base.getNodeKind() == getNodeKind()) {
      const Statement& node = dynamic_cast<const Statement&>(base);
      double matchAttrs = 0;
  ...
  NodeHashType Statement::getHash(std::set<NodeId>& travNodes) const {
    ...
    Crc_type resultHash;
    resultHash.process_bytes( "statm::Statement", strlen("statm::Statement"));
  ...
  void Statement::sort(bool withVirtualBase /* = true */) {
    if (withVirtualBase)
      Base::sort(false);
    if (withVirtualBase)
