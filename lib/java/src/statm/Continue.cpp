...
namespace statm { 
  ...
  void Continue::clone(const Continue& other, bool tryOnVirtualParent) {
  ...
  double Continue::getSimilarity(const base::Base& base){
    if(base.getNodeKind() == getNodeKind()) {
      const Continue& node = dynamic_cast<const Continue&>(base);
      double matchAttrs = 0;
  ...
  NodeHashType Continue::getHash(std::set<NodeId>& travNodes) const {
    ...
    Crc_type resultHash;
    resultHash.process_bytes( "statm::Continue", strlen("statm::Continue"));
  ...
  void Continue::sort(bool withVirtualBase /* = true */) {
    if (withVirtualBase)
      Base::sort(false);
    if (withVirtualBase)
