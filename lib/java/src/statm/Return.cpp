...
namespace statm { 
  ...
  void Return::clone(const Return& other, bool tryOnVirtualParent) {
  ...
  expr::Expression* Return::getExpression() const {
  ...
  void Return::setExpression(expr::Expression *_node) {
    if (_node == NULL)
      throw JavaException(COLUMBUS_LOCATION, CMSG_EX_CAN_T_SET_EDGE_TO_NULL);
  ...
  double Return::getSimilarity(const base::Base& base){
    if(base.getNodeKind() == getNodeKind()) {
      const Return& node = dynamic_cast<const Return&>(base);
      double matchAttrs = 0;
  ...
  NodeHashType Return::getHash(std::set<NodeId>& travNodes) const {
    ...
    Crc_type resultHash;
    resultHash.process_bytes( "statm::Return", strlen("statm::Return"));
  ...
  void Return::sort(bool withVirtualBase /* = true */) {
    if (withVirtualBase)
      Base::sort(false);
    if (withVirtualBase)
