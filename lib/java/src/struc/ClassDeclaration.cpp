...
namespace struc { 
  ClassDeclaration::ClassDeclaration(NodeId _id, Factory *_factory) :
         Base(_id, _factory),
  ...
  void ClassDeclaration::clone(const ClassDeclaration& other, bool tryOnVirtualParent) {
  ...
  void ClassDeclaration::prepareDelete(bool tryOnVirtualParent){
    if (tryOnVirtualParent) {
  ...
  bool ClassDeclaration::setEdge(EdgeKind edgeKind, NodeId edgeEnd, bool tryOnVirtualParent) {
    if (tryOnVirtualParent) {
  ...
  bool ClassDeclaration::removeEdge(EdgeKind edgeKind, NodeId edgeEnd, bool tryOnVirtualParent) {
    if (tryOnVirtualParent) {
  ...
  double ClassDeclaration::getSimilarity(const base::Base& base){
    if(base.getNodeKind() == getNodeKind()) {
      const ClassDeclaration& node = dynamic_cast<const ClassDeclaration&>(base);
      double matchAttrs = 0;
  ...
  void ClassDeclaration::swapStringTable(RefDistributorStrTable& newStrTable, std::map<Key,Key>& oldAndNewStrKeyMap ){
  ...
  NodeHashType ClassDeclaration::getHash(std::set<NodeId>& travNodes) const {
    ...
    Crc_type resultHash;
    resultHash.process_bytes( "struc::ClassDeclaration", strlen("struc::ClassDeclaration"));
  ...
  void ClassDeclaration::sort(bool withVirtualBase /* = true */) {
    if (withVirtualBase)
      Base::sort(false);
    if (withVirtualBase)
  ...
  void ClassDeclaration::save(io::BinaryIO &binIo,bool withVirtualBase  /*= true*/) const {
    if (withVirtualBase)
  ...
  void ClassDeclaration::load(io::BinaryIO &binIo, bool withVirtualBase /*= true*/) {
    if (withVirtualBase)
