...
namespace struc { 
  ...
  void Member::clone(const Member& other, bool tryOnVirtualParent) {
    ...
    struc::AnnotatedElement::clone(other, false);
  ...
  void Member::prepareDelete(bool tryOnVirtualParent){
    ...
    struc::AnnotatedElement::prepareDelete(false);
  }

  bool Member::setEdge(EdgeKind edgeKind, NodeId edgeEnd, bool tryOnVirtualParent) {
    if (tryOnVirtualParent) {
    ...
    if (struc::AnnotatedElement::setEdge(edgeKind, edgeEnd, false)) {
      return true;
    }
    return false;
  }
  ...
  bool Member::removeEdge(EdgeKind edgeKind, NodeId edgeEnd, bool tryOnVirtualParent) {
    ...
    if (struc::AnnotatedElement::removeEdge(edgeKind, edgeEnd, false)) {
      return true;
    }
    return false;
  }
  ...
  NodeHashType Member::getHash(std::set<NodeId>& travNodes) const {
    ...
    Crc_type resultHash;
    resultHash.process_bytes( "struc::Member", strlen("struc::Member"));
  ...
  void Member::sort(bool withVirtualBase /* = true */) {
    if (withVirtualBase)
      Base::sort(false);
    if (withVirtualBase)
      Commentable::sort(false);
    AnnotatedElement::sort(false);
  }
  ...
  void Member::save(io::BinaryIO &binIo,bool withVirtualBase  /*= true*/) const {
    ...
    AnnotatedElement::save(binIo,false);
  ...
  void Member::load(io::BinaryIO &binIo, bool withVirtualBase /*= true*/) {
    ...
    AnnotatedElement::load(binIo,false);
