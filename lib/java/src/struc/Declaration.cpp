...
namespace struc { 
  ...
  void Declaration::clone(const Declaration& other, bool tryOnVirtualParent) {
  ...
  double Declaration::getSimilarity(const base::Base& base){
    if(base.getNodeKind() == getNodeKind()) {
      const Declaration& node = dynamic_cast<const Declaration&>(base);
      double matchAttrs = 0;
  ...
  NodeHashType Declaration::getHash(std::set<NodeId>& travNodes) const {
    ...
    Crc_type resultHash;
    resultHash.process_bytes( "struc::Declaration", strlen("struc::Declaration"));
  ...
  void Declaration::sort(bool withVirtualBase /* = true */) {
    if (withVirtualBase)
      Base::sort(false);
    if (withVirtualBase)
