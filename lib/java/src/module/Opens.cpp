...
namespace module { 
  Opens::Opens(NodeId _id, Factory *_factory) :
         Base(_id, _factory),
  ...
  void Opens::clone(const Opens& other, bool tryOnVirtualParent) {
  ...
  void Opens::prepareDelete(bool tryOnVirtualParent){
    removePackageName();
    while (!hasModuleNamesContainer.empty()) {
      const NodeId id = *hasModuleNamesContainer.begin();
      removeParentEdge(id);
      if (factory->getExistsReverseEdges())
        factory->reverseEdges->removeEdge(id, this->getId(), edkOpens_HasModuleNames);
  ...
  NodeKind Opens::getNodeKind() const {
    return ndkOpens;
  }

  expr::Expression* Opens::getPackageName() const {
  ...
  ListIterator<expr::Expression> Opens::getModuleNamesListIteratorBegin() const {
    return ListIterator<expr::Expression>(&hasModuleNamesContainer, factory, true);
  }

  ListIterator<expr::Expression> Opens::getModuleNamesListIteratorEnd() const {
    return ListIterator<expr::Expression>(&hasModuleNamesContainer, factory, false);
  }

  bool Opens::getModuleNamesIsEmpty() const {
    return getModuleNamesListIteratorBegin() == getModuleNamesListIteratorEnd();
  }

  unsigned int Opens::getModuleNamesSize() const {
    unsigned int size = 0;
  ...
  bool Opens::setEdge(EdgeKind edgeKind, NodeId edgeEnd, bool tryOnVirtualParent) {
    switch (edgeKind) {
      case edkOpens_HasPackageName:
        setPackageName(edgeEnd);
        return true;
      case edkOpens_HasModuleNames:
  ...
  bool Opens::removeEdge(EdgeKind edgeKind, NodeId edgeEnd, bool tryOnVirtualParent) {
    switch (edgeKind) {
      case edkOpens_HasPackageName:
        removePackageName();
        return true;
  ...
  void Opens::setPackageName(NodeId _id) {
    expr::Expression *_node = NULL;
    if (_id) {
      ...
      if (m_hasPackageName) {
        ...
        if (factory->getExistsReverseEdges())
          factory->reverseEdges->removeEdge(m_hasPackageName, m_id, edkOpens_HasPackageName);
      }
      m_hasPackageName = _node->getId();
      if (m_hasPackageName != 0)
        setParentEdge(factory->getPointer(m_hasPackageName), edkOpens_HasPackageName);
      if (factory->getExistsReverseEdges())
        factory->reverseEdges->insertEdge(m_hasPackageName, this->getId(), edkOpens_HasPackageName);
    } else {
      if (m_hasPackageName) {
  ...
  void Opens::setPackageName(expr::Expression *_node) {
    if (_node == NULL)
      throw JavaException(COLUMBUS_LOCATION, CMSG_EX_CAN_T_SET_EDGE_TO_NULL);
  ...
  void Opens::addModuleNames(const expr::Expression *_node) {
    if (_node == NULL)
    ...
    setParentEdge(_node,edkOpens_HasModuleNames);

    if (factory->reverseEdges)
      factory->reverseEdges->insertEdge(_node, this, edkOpens_HasModuleNames);
  }

  void Opens::addModuleNames(NodeId _id) {
  ...
  void Opens::removeModuleNames(NodeId id) {
  ...
  void Opens::removeModuleNames(expr::Expression *_node) {
    if (_node == NULL)
  ...
  void Opens::accept(Visitor &visitor) const {
    visitor.visit(*this);
  }

  void Opens::acceptEnd(Visitor &visitor) const {
    visitor.visitEnd(*this);
  }

  double Opens::getSimilarity(const base::Base& base){
    if(base.getNodeKind() == getNodeKind()) {
      const Opens& node = dynamic_cast<const Opens&>(base);
      double matchAttrs = 0;
  ...
  void Opens::swapStringTable(RefDistributorStrTable& newStrTable, std::map<Key,Key>& oldAndNewStrKeyMap ){
    std::map<Key,Key>::iterator foundKeyId;

  }

  NodeHashType Opens::getHash(std::set<NodeId>& travNodes) const {
    ...
    Crc_type resultHash;
    resultHash.process_bytes( "module::Opens", strlen("module::Opens"));
  ...
  void Opens::sort(bool withVirtualBase /* = true */) {
    if (withVirtualBase)
      Base::sort(false);
    if (withVirtualBase)
  ...
  void Opens::save(io::BinaryIO &binIo,bool withVirtualBase  /*= true*/) const {
    if (withVirtualBase)
  ...
  void Opens::load(io::BinaryIO &binIo, bool withVirtualBase /*= true*/) {
    if (withVirtualBase)
    ...
    while (_id) {
      ...
      setParentEdge(factory->getPointer(_id),edkOpens_HasModuleNames);
      _id = binIo.readUInt4();
    }
  }
