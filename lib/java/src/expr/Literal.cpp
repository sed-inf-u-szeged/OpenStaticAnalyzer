...
namespace expr { 
  ...
  void Literal::clone(const Literal& other, bool tryOnVirtualParent) {
  ...
  NodeHashType Literal::getHash(std::set<NodeId>& travNodes) const {
    ...
    Crc_type resultHash;
    resultHash.process_bytes( "expr::Literal", strlen("expr::Literal"));
  ...
  void Literal::sort(bool withVirtualBase /* = true */) {
    if (withVirtualBase)
      Base::sort(false);
    if (withVirtualBase)
