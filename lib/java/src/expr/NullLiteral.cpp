...
namespace expr { 
  ...
  void NullLiteral::clone(const NullLiteral& other, bool tryOnVirtualParent) {
  ...
  NodeHashType NullLiteral::getHash(std::set<NodeId>& travNodes) const {
    ...
    Crc_type resultHash;
    resultHash.process_bytes( "expr::NullLiteral", strlen("expr::NullLiteral"));
  ...
  void NullLiteral::sort(bool withVirtualBase /* = true */) {
    if (withVirtualBase)
      Base::sort(false);
    if (withVirtualBase)
