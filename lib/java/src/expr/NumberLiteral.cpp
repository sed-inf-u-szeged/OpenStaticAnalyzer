...
namespace expr { 
  ...
  void NumberLiteral::clone(const NumberLiteral& other, bool tryOnVirtualParent) {
  ...
  Key NumberLiteral::getValueKey() const {
    return m_value;
  }

  const std::string& NumberLiteral::getValue() const {
    return factory->getStringTable().get(m_value);
  }

  void NumberLiteral::setValueKey(Key _value) {
    m_value = _value;
  }

  void NumberLiteral::setValue(const std::string& _value) {
  ...
  NodeHashType NumberLiteral::getHash(std::set<NodeId>& travNodes) const {
    ...
    Crc_type resultHash;
    resultHash.process_bytes( "expr::NumberLiteral", strlen("expr::NumberLiteral"));
  ...
  void NumberLiteral::sort(bool withVirtualBase /* = true */) {
    if (withVirtualBase)
      Base::sort(false);
    if (withVirtualBase)
