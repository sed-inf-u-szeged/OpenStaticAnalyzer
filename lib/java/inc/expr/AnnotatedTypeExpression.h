...
#ifndef _JAVA_AnnotatedTypeExpression_H_
#define _JAVA_AnnotatedTypeExpression_H_

#include "java/inc/java.h"

/**
* \file AnnotatedTypeExpression.h
* \brief Contains declaration of the expr::AnnotatedTypeExpression class.
...
namespace expr {
  ...
  /**
  * \brief AnnotatedTypeExpression class, which represents the expr::AnnotatedTypeExpression node.
  * (missing)
  * 
  * Edges:
  *   - hasAnnotations (expr::Annotation, multiple) : (missing)
  *   - hasUnderlyingType (expr::TypeExpression, single) : (missing)
  */
  class AnnotatedTypeExpression : public TypeExpression {
    protected:
      /**
      * \internal
      ...
      */
      AnnotatedTypeExpression(NodeId nodeId, Factory *factory);

      /**
      * \internal
      * \brief Non-public destructor, only factory can destroy nodes.
      */
      virtual ~AnnotatedTypeExpression();

    private:
      /**
      * \brief This function always throws a JavaException due to copying is not allowed!
      */
      AnnotatedTypeExpression & operator=(const AnnotatedTypeExpression&);

      /**
      * \brief This function always throws a JavaException due to copying is not allowed!
      */
      AnnotatedTypeExpression(const AnnotatedTypeExpression&);
    ...
    protected:
      ...
      * \brief Clone method.
      */
      virtual void clone(const AnnotatedTypeExpression& other, bool tryOnVirtualParent);
    ...
    public:
      ...
      */
      ...
      /**
      * \brief Gives back the pointer of the node the hasUnderlyingType edge points to.
      * \return Returns the end point of the hasUnderlyingType edge.
      */
      expr::TypeExpression* getUnderlyingType() const;
      ...
      */
      void addAnnotations(const Annotation *node);
      ...
      */
      void removeAnnotations(Annotation *node);

      /**
      * \brief Sets the hasUnderlyingType edge.
      * \param id [in] The new end point of the hasUnderlyingType edge.
      */
      void setUnderlyingType(NodeId id);

      /**
      * \brief Sets the hasUnderlyingType edge.
      * \param node [in] The new end point of the hasUnderlyingType edge.
      */
      void setUnderlyingType(TypeExpression *node);

      /**
      * \brief remove the hasUnderlyingType edge.
      */
      void removeUnderlyingType();
    ...
    protected:
      ...
      /** \internal \brief The id of the node the hasUnderlyingType edge points to. */
      NodeId m_hasUnderlyingType;
