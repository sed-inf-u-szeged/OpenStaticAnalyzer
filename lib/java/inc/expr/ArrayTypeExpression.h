...
#ifndef _JAVA_ArrayTypeExpression_H_
#define _JAVA_ArrayTypeExpression_H_

#include "java/inc/java.h"

/**
* \file ArrayTypeExpression.h
* \brief Contains declaration of the expr::ArrayTypeExpression class.
...
namespace expr {
  ...
  /**
  * \brief ArrayTypeExpression class, which represents the expr::ArrayTypeExpression node.
  * (missing)
  * 
  * Edges:
  *   - hasComponentType (expr::TypeExpression, single) : (missing)
  */
  class ArrayTypeExpression : public TypeExpression {
    protected:
      /**
      * \internal
      ...
      */
      ArrayTypeExpression(NodeId nodeId, Factory *factory);

      /**
      * \internal
      * \brief Non-public destructor, only factory can destroy nodes.
      */
      virtual ~ArrayTypeExpression();

    private:
      /**
      * \brief This function always throws a JavaException due to copying is not allowed!
      */
      ArrayTypeExpression & operator=(const ArrayTypeExpression&);

      /**
      * \brief This function always throws a JavaException due to copying is not allowed!
      */
      ArrayTypeExpression(const ArrayTypeExpression&);
    ...
    protected:
      ...
      * \brief Clone method.
      */
      virtual void clone(const ArrayTypeExpression& other, bool tryOnVirtualParent);
