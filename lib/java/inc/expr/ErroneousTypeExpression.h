...
#ifndef _JAVA_ErroneousTypeExpression_H_
#define _JAVA_ErroneousTypeExpression_H_

#include "java/inc/java.h"

/**
* \file ErroneousTypeExpression.h
* \brief Contains declaration of the expr::ErroneousTypeExpression class.
...
namespace expr {
  ...
  /**
  * \brief ErroneousTypeExpression class, which represents the expr::ErroneousTypeExpression node.
  * (missing)
  * 
  * Edges:
  *   - hasErrors (base::Positioned, multiple) : (missing)
  */
  class ErroneousTypeExpression : public TypeExpression {
    protected:
      /**
      * \internal
      ...
      */
      ErroneousTypeExpression(NodeId nodeId, Factory *factory);

      /**
      * \internal
      * \brief Non-public destructor, only factory can destroy nodes.
      */
      virtual ~ErroneousTypeExpression();

    private:
      /**
      * \brief This function always throws a JavaException due to copying is not allowed!
      */
      ErroneousTypeExpression & operator=(const ErroneousTypeExpression&);

      /**
      * \brief This function always throws a JavaException due to copying is not allowed!
      */
      ErroneousTypeExpression(const ErroneousTypeExpression&);
    ...
    protected:
      ...
      * \brief Clone method.
      */
      virtual void clone(const ErroneousTypeExpression& other, bool tryOnVirtualParent);
