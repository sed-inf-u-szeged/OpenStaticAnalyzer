...
namespace statement { 
  ...
  expression::Expression* ExpressionStatement::getExpression() const {
  ...
  void ExpressionStatement::setExpression(expression::Expression *_node) {
    if (_node == NULL)
      throw JavaScriptException(COLUMBUS_LOCATION, CMSG_EX_CAN_T_SET_EDGE_TO_NULL);
  ...
  NodeHashType ExpressionStatement::getHash(std::set<NodeId>& travNodes) const {
    ...
    Crc_type resultHash;
    resultHash.process_bytes( "statement::ExpressionStatement", strlen("statement::ExpressionStatement"));
