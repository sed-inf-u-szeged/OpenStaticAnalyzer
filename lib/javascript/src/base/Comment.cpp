...
namespace base { 
  ...
  CommentLocation Comment::getLocation() const {
    return m_location;
  }
  ...
  void Comment::setType(CommentType _type) {
    m_type = _type;
  }

  void Comment::setLocation(CommentLocation _location) {
  ...
  double Comment::getSimilarity(const base::Base& base){
    if(base.getNodeKind() == getNodeKind()) {
      ...
      if(node.getLocation() == getLocation()) ++matchAttrs;
  ...
  void Comment::load(io::BinaryIO &binIo, bool withVirtualBase /*= true*/) {
    ...
    m_type = (CommentType)binIo.readUByte1();
    m_location = (CommentLocation)binIo.readUByte1();
