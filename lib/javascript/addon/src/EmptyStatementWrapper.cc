...
napi_ref EmptyStatementWrapper::constructor;

EmptyStatementWrapper::EmptyStatementWrapper(): env_(nullptr), wrapper_(nullptr) {}

EmptyStatementWrapper::~EmptyStatementWrapper(){ napi_delete_reference(env_, wrapper_); }

void EmptyStatementWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  EmptyStatementWrapper* obj = reinterpret_cast<EmptyStatementWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~EmptyStatementWrapper();
}
napi_value EmptyStatementWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  ...
  napi_value cons;
  status = napi_define_class(env, "EmptyStatementWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value EmptyStatementWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  EmptyStatementWrapper* obj = new EmptyStatementWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), EmptyStatementWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status EmptyStatementWrapper::NewInstance(napi_env env, statement::EmptyStatement* arg, napi_value* instance) {
  ...
  EmptyStatementWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value EmptyStatementWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::statement::EmptyStatement* source = dynamic_cast<columbus::javascript::asg::statement::EmptyStatement*>(obj->_nativeObj);
  columbus::javascript::asg::base::Comment* target = dynamic_cast<columbus::javascript::asg::base::Comment*>(param->_nativeObj);

  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast statement::EmptyStatement" );
  }
  if(target == nullptr){
...
napi_value EmptyStatementWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value EmptyStatementWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
