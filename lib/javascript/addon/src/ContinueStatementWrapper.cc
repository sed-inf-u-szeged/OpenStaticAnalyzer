...
napi_ref ContinueStatementWrapper::constructor;

ContinueStatementWrapper::ContinueStatementWrapper(): env_(nullptr), wrapper_(nullptr) {}

ContinueStatementWrapper::~ContinueStatementWrapper(){ napi_delete_reference(env_, wrapper_); }

void ContinueStatementWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  ContinueStatementWrapper* obj = reinterpret_cast<ContinueStatementWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~ContinueStatementWrapper();
}
napi_value ContinueStatementWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  ...
  napi_value cons;
  status = napi_define_class(env, "ContinueStatementWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value ContinueStatementWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  ContinueStatementWrapper* obj = new ContinueStatementWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), ContinueStatementWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status ContinueStatementWrapper::NewInstance(napi_env env, statement::ContinueStatement* arg, napi_value* instance) {
  ...
  ContinueStatementWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value ContinueStatementWrapper::setLabel(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::statement::ContinueStatement* source = dynamic_cast<columbus::javascript::asg::statement::ContinueStatement*>(obj->_nativeObj);
  ...
  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast statement::ContinueStatement" );
  }
  if(target == nullptr){
...
napi_value ContinueStatementWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ContinueStatementWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ContinueStatementWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
