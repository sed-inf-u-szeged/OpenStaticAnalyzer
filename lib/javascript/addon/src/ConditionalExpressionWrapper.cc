...
napi_ref ConditionalExpressionWrapper::constructor;

ConditionalExpressionWrapper::ConditionalExpressionWrapper(): env_(nullptr), wrapper_(nullptr) {}

ConditionalExpressionWrapper::~ConditionalExpressionWrapper(){ napi_delete_reference(env_, wrapper_); }

void ConditionalExpressionWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  ConditionalExpressionWrapper* obj = reinterpret_cast<ConditionalExpressionWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~ConditionalExpressionWrapper();
}
napi_value ConditionalExpressionWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  napi_property_descriptor props [] = {
  DECLARE_NAPI_METHOD( "setAlternate", setAlternate),
  DECLARE_NAPI_METHOD( "setConsequent", setConsequent),
  DECLARE_NAPI_METHOD( "setTest", setTest),
  ...
  napi_value cons;
  status = napi_define_class(env, "ConditionalExpressionWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value ConditionalExpressionWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  ConditionalExpressionWrapper* obj = new ConditionalExpressionWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), ConditionalExpressionWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status ConditionalExpressionWrapper::NewInstance(napi_env env, expression::ConditionalExpression* arg, napi_value* instance) {
  ...
  ConditionalExpressionWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value ConditionalExpressionWrapper::setAlternate(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::expression::ConditionalExpression* source = dynamic_cast<columbus::javascript::asg::expression::ConditionalExpression*>(obj->_nativeObj);
  ...
  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast expression::ConditionalExpression" );
  }
  if(target == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast expression::Expression" );
  }

  source->setAlternate(target);
  return nullptr;
}
napi_value ConditionalExpressionWrapper::setConsequent(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  source->setConsequent(target);
  return nullptr;
}
napi_value ConditionalExpressionWrapper::setTest(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  source->setTest(target);
  return nullptr;
}
napi_value ConditionalExpressionWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ConditionalExpressionWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ConditionalExpressionWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
