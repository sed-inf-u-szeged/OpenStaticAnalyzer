...
napi_ref AssignmentPatternWrapper::constructor;

AssignmentPatternWrapper::AssignmentPatternWrapper(): env_(nullptr), wrapper_(nullptr) {}

AssignmentPatternWrapper::~AssignmentPatternWrapper(){ napi_delete_reference(env_, wrapper_); }

void AssignmentPatternWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  AssignmentPatternWrapper* obj = reinterpret_cast<AssignmentPatternWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~AssignmentPatternWrapper();
}
napi_value AssignmentPatternWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  ...
  napi_value cons;
  status = napi_define_class(env, "AssignmentPatternWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value AssignmentPatternWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  AssignmentPatternWrapper* obj = new AssignmentPatternWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), AssignmentPatternWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status AssignmentPatternWrapper::NewInstance(napi_env env, statement::AssignmentPattern* arg, napi_value* instance) {
  ...
  AssignmentPatternWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value AssignmentPatternWrapper::setLeft(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::statement::AssignmentPattern* source = dynamic_cast<columbus::javascript::asg::statement::AssignmentPattern*>(obj->_nativeObj);
  columbus::javascript::asg::statement::Pattern* target = dynamic_cast<columbus::javascript::asg::statement::Pattern*>(param->_nativeObj);

  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast statement::AssignmentPattern" );
  }
  if(target == nullptr){
...
napi_value AssignmentPatternWrapper::setRight(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value AssignmentPatternWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value AssignmentPatternWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value AssignmentPatternWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
