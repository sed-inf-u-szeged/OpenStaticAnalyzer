...
napi_ref SuperWrapper::constructor;

SuperWrapper::SuperWrapper(): env_(nullptr), wrapper_(nullptr) {}

SuperWrapper::~SuperWrapper(){ napi_delete_reference(env_, wrapper_); }

void SuperWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  SuperWrapper* obj = reinterpret_cast<SuperWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~SuperWrapper();
}
napi_value SuperWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  ...
  napi_value cons;
  status = napi_define_class(env, "SuperWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value SuperWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  SuperWrapper* obj = new SuperWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), SuperWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status SuperWrapper::NewInstance(napi_env env, expression::Super* arg, napi_value* instance) {
...
napi_value SuperWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::expression::Super* source = dynamic_cast<columbus::javascript::asg::expression::Super*>(obj->_nativeObj);
  columbus::javascript::asg::base::Comment* target = dynamic_cast<columbus::javascript::asg::base::Comment*>(param->_nativeObj);

  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast expression::Super" );
  }
  if(target == nullptr){
...
napi_value SuperWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value SuperWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
