...
napi_ref ObjectExpressionWrapper::constructor;

ObjectExpressionWrapper::ObjectExpressionWrapper(): env_(nullptr), wrapper_(nullptr) {}

ObjectExpressionWrapper::~ObjectExpressionWrapper(){ napi_delete_reference(env_, wrapper_); }

void ObjectExpressionWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  ObjectExpressionWrapper* obj = reinterpret_cast<ObjectExpressionWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~ObjectExpressionWrapper();
}
napi_value ObjectExpressionWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  napi_property_descriptor props [] = {
  DECLARE_NAPI_METHOD( "addProperties", addProperties),
  ...
  napi_value cons;
  status = napi_define_class(env, "ObjectExpressionWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value ObjectExpressionWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  ObjectExpressionWrapper* obj = new ObjectExpressionWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), ObjectExpressionWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status ObjectExpressionWrapper::NewInstance(napi_env env, expression::ObjectExpression* arg, napi_value* instance) {
  ...
  ObjectExpressionWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value ObjectExpressionWrapper::addProperties(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::expression::ObjectExpression* source = dynamic_cast<columbus::javascript::asg::expression::ObjectExpression*>(obj->_nativeObj);
  columbus::javascript::asg::base::Positioned* target = dynamic_cast<columbus::javascript::asg::base::Positioned*>(param->_nativeObj);

  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast expression::ObjectExpression" );
  }
  if(target == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast base::Positioned" );
  }

  source->addProperties(target);
  return nullptr;
}
napi_value ObjectExpressionWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ObjectExpressionWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ObjectExpressionWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
