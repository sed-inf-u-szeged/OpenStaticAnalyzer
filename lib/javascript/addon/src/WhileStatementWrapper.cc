...
napi_ref WhileStatementWrapper::constructor;

WhileStatementWrapper::WhileStatementWrapper(): env_(nullptr), wrapper_(nullptr) {}

WhileStatementWrapper::~WhileStatementWrapper(){ napi_delete_reference(env_, wrapper_); }

void WhileStatementWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  WhileStatementWrapper* obj = reinterpret_cast<WhileStatementWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~WhileStatementWrapper();
}
napi_value WhileStatementWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  ...
  napi_value cons;
  status = napi_define_class(env, "WhileStatementWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value WhileStatementWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  WhileStatementWrapper* obj = new WhileStatementWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), WhileStatementWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status WhileStatementWrapper::NewInstance(napi_env env, statement::WhileStatement* arg, napi_value* instance) {
  ...
  WhileStatementWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value WhileStatementWrapper::setBody(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::statement::WhileStatement* source = dynamic_cast<columbus::javascript::asg::statement::WhileStatement*>(obj->_nativeObj);
  ...
  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast statement::WhileStatement" );
  }
  if(target == nullptr){
...
napi_value WhileStatementWrapper::setTest(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value WhileStatementWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value WhileStatementWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value WhileStatementWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
