...
napi_ref DoWhileStatementWrapper::constructor;

DoWhileStatementWrapper::DoWhileStatementWrapper(): env_(nullptr), wrapper_(nullptr) {}

DoWhileStatementWrapper::~DoWhileStatementWrapper(){ napi_delete_reference(env_, wrapper_); }

void DoWhileStatementWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  DoWhileStatementWrapper* obj = reinterpret_cast<DoWhileStatementWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~DoWhileStatementWrapper();
}
napi_value DoWhileStatementWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  ...
  napi_value cons;
  status = napi_define_class(env, "DoWhileStatementWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value DoWhileStatementWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  DoWhileStatementWrapper* obj = new DoWhileStatementWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), DoWhileStatementWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status DoWhileStatementWrapper::NewInstance(napi_env env, statement::DoWhileStatement* arg, napi_value* instance) {
  ...
  DoWhileStatementWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value DoWhileStatementWrapper::setBody(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::statement::DoWhileStatement* source = dynamic_cast<columbus::javascript::asg::statement::DoWhileStatement*>(obj->_nativeObj);
  ...
  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast statement::DoWhileStatement" );
  }
  if(target == nullptr){
...
napi_value DoWhileStatementWrapper::setTest(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value DoWhileStatementWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value DoWhileStatementWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value DoWhileStatementWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
