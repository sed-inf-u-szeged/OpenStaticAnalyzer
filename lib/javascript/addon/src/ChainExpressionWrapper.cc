...
napi_ref ChainExpressionWrapper::constructor;

ChainExpressionWrapper::ChainExpressionWrapper(): env_(nullptr), wrapper_(nullptr) {}

ChainExpressionWrapper::~ChainExpressionWrapper(){ napi_delete_reference(env_, wrapper_); }

void ChainExpressionWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  ChainExpressionWrapper* obj = reinterpret_cast<ChainExpressionWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~ChainExpressionWrapper();
}
napi_value ChainExpressionWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  ...
  napi_value cons;
  status = napi_define_class(env, "ChainExpressionWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value ChainExpressionWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  ChainExpressionWrapper* obj = new ChainExpressionWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), ChainExpressionWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status ChainExpressionWrapper::NewInstance(napi_env env, expression::ChainExpression* arg, napi_value* instance) {
  ...
  ChainExpressionWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value ChainExpressionWrapper::setExpression(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::expression::ChainExpression* source = dynamic_cast<columbus::javascript::asg::expression::ChainExpression*>(obj->_nativeObj);
  columbus::javascript::asg::expression::ChainElement* target = dynamic_cast<columbus::javascript::asg::expression::ChainElement*>(param->_nativeObj);

  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast expression::ChainExpression" );
  }
  if(target == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast expression::ChainElement" );
  }

  source->setExpression(target);
  return nullptr;
}
napi_value ChainExpressionWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ChainExpressionWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ChainExpressionWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
