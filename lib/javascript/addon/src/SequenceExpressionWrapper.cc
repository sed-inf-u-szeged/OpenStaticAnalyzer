...
napi_ref SequenceExpressionWrapper::constructor;

SequenceExpressionWrapper::SequenceExpressionWrapper(): env_(nullptr), wrapper_(nullptr) {}

SequenceExpressionWrapper::~SequenceExpressionWrapper(){ napi_delete_reference(env_, wrapper_); }

void SequenceExpressionWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  SequenceExpressionWrapper* obj = reinterpret_cast<SequenceExpressionWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~SequenceExpressionWrapper();
}
napi_value SequenceExpressionWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  ...
  napi_value cons;
  status = napi_define_class(env, "SequenceExpressionWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value SequenceExpressionWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  SequenceExpressionWrapper* obj = new SequenceExpressionWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), SequenceExpressionWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status SequenceExpressionWrapper::NewInstance(napi_env env, expression::SequenceExpression* arg, napi_value* instance) {
  ...
  SequenceExpressionWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value SequenceExpressionWrapper::addExpressions(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::expression::SequenceExpression* source = dynamic_cast<columbus::javascript::asg::expression::SequenceExpression*>(obj->_nativeObj);
  ...
  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast expression::SequenceExpression" );
  }
  if(target == nullptr){
...
napi_value SequenceExpressionWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value SequenceExpressionWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value SequenceExpressionWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
