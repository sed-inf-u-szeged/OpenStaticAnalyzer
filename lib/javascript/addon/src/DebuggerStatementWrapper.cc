...
napi_ref DebuggerStatementWrapper::constructor;

DebuggerStatementWrapper::DebuggerStatementWrapper(): env_(nullptr), wrapper_(nullptr) {}

DebuggerStatementWrapper::~DebuggerStatementWrapper(){ napi_delete_reference(env_, wrapper_); }

void DebuggerStatementWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  DebuggerStatementWrapper* obj = reinterpret_cast<DebuggerStatementWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~DebuggerStatementWrapper();
}
napi_value DebuggerStatementWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  ...
  napi_value cons;
  status = napi_define_class(env, "DebuggerStatementWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value DebuggerStatementWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  DebuggerStatementWrapper* obj = new DebuggerStatementWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), DebuggerStatementWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status DebuggerStatementWrapper::NewInstance(napi_env env, statement::DebuggerStatement* arg, napi_value* instance) {
  ...
  DebuggerStatementWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value DebuggerStatementWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::statement::DebuggerStatement* source = dynamic_cast<columbus::javascript::asg::statement::DebuggerStatement*>(obj->_nativeObj);
  columbus::javascript::asg::base::Comment* target = dynamic_cast<columbus::javascript::asg::base::Comment*>(param->_nativeObj);

  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast statement::DebuggerStatement" );
  }
  if(target == nullptr){
...
napi_value DebuggerStatementWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value DebuggerStatementWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
