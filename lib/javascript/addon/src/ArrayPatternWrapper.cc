...
napi_ref ArrayPatternWrapper::constructor;

ArrayPatternWrapper::ArrayPatternWrapper(): env_(nullptr), wrapper_(nullptr) {}

ArrayPatternWrapper::~ArrayPatternWrapper(){ napi_delete_reference(env_, wrapper_); }

void ArrayPatternWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  ArrayPatternWrapper* obj = reinterpret_cast<ArrayPatternWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~ArrayPatternWrapper();
}
napi_value ArrayPatternWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  ...
  napi_value cons;
  status = napi_define_class(env, "ArrayPatternWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value ArrayPatternWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  ArrayPatternWrapper* obj = new ArrayPatternWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), ArrayPatternWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status ArrayPatternWrapper::NewInstance(napi_env env, statement::ArrayPattern* arg, napi_value* instance) {
  ...
  ArrayPatternWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value ArrayPatternWrapper::addElements(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::statement::ArrayPattern* source = dynamic_cast<columbus::javascript::asg::statement::ArrayPattern*>(obj->_nativeObj);
  columbus::javascript::asg::statement::Pattern* target = dynamic_cast<columbus::javascript::asg::statement::Pattern*>(param->_nativeObj);

  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast statement::ArrayPattern" );
  }
  if(target == nullptr){
...
napi_value ArrayPatternWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ArrayPatternWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ArrayPatternWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
