...
napi_ref ProgramWrapper::constructor;

ProgramWrapper::ProgramWrapper(): env_(nullptr), wrapper_(nullptr) {}

ProgramWrapper::~ProgramWrapper(){ napi_delete_reference(env_, wrapper_); }

void ProgramWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  ProgramWrapper* obj = reinterpret_cast<ProgramWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~ProgramWrapper();
}
napi_value ProgramWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  napi_property_descriptor props [] = {
  DECLARE_NAPI_METHOD( "addBody", addBody),
  DECLARE_NAPI_METHOD( "addComments", addComments),
    DECLARE_NAPI_METHOD("setSourceType", setSourceType),
  ...
  napi_value cons;
  status = napi_define_class(env, "ProgramWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value ProgramWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  ProgramWrapper* obj = new ProgramWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), ProgramWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status ProgramWrapper::NewInstance(napi_env env, base::Program* arg, napi_value* instance) {
...
napi_value ProgramWrapper::addBody(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::base::Program* source = dynamic_cast<columbus::javascript::asg::base::Program*>(obj->_nativeObj);
...
napi_value ProgramWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ProgramWrapper::setSourceType(napi_env env, napi_callback_info info){ 
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  if( param == "pstScript" ){
    dynamic_cast<columbus::javascript::asg::base::Program*>(obj->_nativeObj)->setSourceType( pstScript );
  }
  if( param == "pstModule" ){
    dynamic_cast<columbus::javascript::asg::base::Program*>(obj->_nativeObj)->setSourceType( pstModule );
  }
  return nullptr;
}
napi_value ProgramWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ProgramWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
...
napi_value ProgramWrapper::setName(napi_env env, napi_callback_info info){ 
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
