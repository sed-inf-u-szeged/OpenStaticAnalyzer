...
napi_ref ImportSpecifierWrapper::constructor;

ImportSpecifierWrapper::ImportSpecifierWrapper(): env_(nullptr), wrapper_(nullptr) {}

ImportSpecifierWrapper::~ImportSpecifierWrapper(){ napi_delete_reference(env_, wrapper_); }

void ImportSpecifierWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  ImportSpecifierWrapper* obj = reinterpret_cast<ImportSpecifierWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~ImportSpecifierWrapper();
}
napi_value ImportSpecifierWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  napi_property_descriptor props [] = {
  DECLARE_NAPI_METHOD( "setImported", setImported),
  ...
  napi_value cons;
  status = napi_define_class(env, "ImportSpecifierWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value ImportSpecifierWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  ImportSpecifierWrapper* obj = new ImportSpecifierWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), ImportSpecifierWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status ImportSpecifierWrapper::NewInstance(napi_env env, structure::ImportSpecifier* arg, napi_value* instance) {
  ...
  ImportSpecifierWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value ImportSpecifierWrapper::setImported(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::structure::ImportSpecifier* source = dynamic_cast<columbus::javascript::asg::structure::ImportSpecifier*>(obj->_nativeObj);
  ...
  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast structure::ImportSpecifier" );
  }
  if(target == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast expression::Identifier" );
  }

  source->setImported(target);
  return nullptr;
}
napi_value ImportSpecifierWrapper::setLocal(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ImportSpecifierWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ImportSpecifierWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ImportSpecifierWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
