...
napi_ref ThrowStatementWrapper::constructor;

ThrowStatementWrapper::ThrowStatementWrapper(): env_(nullptr), wrapper_(nullptr) {}

ThrowStatementWrapper::~ThrowStatementWrapper(){ napi_delete_reference(env_, wrapper_); }

void ThrowStatementWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  ThrowStatementWrapper* obj = reinterpret_cast<ThrowStatementWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~ThrowStatementWrapper();
}
napi_value ThrowStatementWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  ...
  napi_value cons;
  status = napi_define_class(env, "ThrowStatementWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value ThrowStatementWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  ThrowStatementWrapper* obj = new ThrowStatementWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), ThrowStatementWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status ThrowStatementWrapper::NewInstance(napi_env env, statement::ThrowStatement* arg, napi_value* instance) {
  ...
  ThrowStatementWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value ThrowStatementWrapper::setArgument(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::statement::ThrowStatement* source = dynamic_cast<columbus::javascript::asg::statement::ThrowStatement*>(obj->_nativeObj);
  ...
  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast statement::ThrowStatement" );
  }
  if(target == nullptr){
...
napi_value ThrowStatementWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ThrowStatementWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ThrowStatementWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
