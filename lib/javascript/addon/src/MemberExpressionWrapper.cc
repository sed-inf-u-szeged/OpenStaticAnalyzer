...
napi_ref MemberExpressionWrapper::constructor;

MemberExpressionWrapper::MemberExpressionWrapper(): env_(nullptr), wrapper_(nullptr) {}

MemberExpressionWrapper::~MemberExpressionWrapper(){ napi_delete_reference(env_, wrapper_); }

void MemberExpressionWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  MemberExpressionWrapper* obj = reinterpret_cast<MemberExpressionWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~MemberExpressionWrapper();
}
napi_value MemberExpressionWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  napi_property_descriptor props [] = {
  DECLARE_NAPI_METHOD( "setProperty", setProperty),
  DECLARE_NAPI_METHOD( "setObject", setObject),
  ...
  napi_value cons;
  status = napi_define_class(env, "MemberExpressionWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value MemberExpressionWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  MemberExpressionWrapper* obj = new MemberExpressionWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), MemberExpressionWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status MemberExpressionWrapper::NewInstance(napi_env env, expression::MemberExpression* arg, napi_value* instance) {
...
napi_value MemberExpressionWrapper::setProperty(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value MemberExpressionWrapper::setObject(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  source->setObject(target);
  return nullptr;
}
napi_value MemberExpressionWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value MemberExpressionWrapper::setComputed(napi_env env, napi_callback_info info){ 
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  assert(status == napi_ok);
  dynamic_cast<columbus::javascript::asg::expression::MemberExpression*>(obj->_nativeObj)->setComputed( b );
  return nullptr;
}
napi_value MemberExpressionWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value MemberExpressionWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
