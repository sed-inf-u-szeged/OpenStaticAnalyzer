...
napi_ref ClassDeclarationWrapper::constructor;

ClassDeclarationWrapper::ClassDeclarationWrapper(): env_(nullptr), wrapper_(nullptr) {}

ClassDeclarationWrapper::~ClassDeclarationWrapper(){ napi_delete_reference(env_, wrapper_); }

void ClassDeclarationWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  ClassDeclarationWrapper* obj = reinterpret_cast<ClassDeclarationWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~ClassDeclarationWrapper();
}
napi_value ClassDeclarationWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  ...
  napi_value cons;
  status = napi_define_class(env, "ClassDeclarationWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value ClassDeclarationWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  ClassDeclarationWrapper* obj = new ClassDeclarationWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), ClassDeclarationWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status ClassDeclarationWrapper::NewInstance(napi_env env, declaration::ClassDeclaration* arg, napi_value* instance) {
  ...
  ClassDeclarationWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value ClassDeclarationWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::declaration::ClassDeclaration* source = dynamic_cast<columbus::javascript::asg::declaration::ClassDeclaration*>(obj->_nativeObj);
  columbus::javascript::asg::base::Comment* target = dynamic_cast<columbus::javascript::asg::base::Comment*>(param->_nativeObj);

  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast declaration::ClassDeclaration" );
  }
  if(target == nullptr){
...
napi_value ClassDeclarationWrapper::setBody(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ClassDeclarationWrapper::setSuperClass(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ClassDeclarationWrapper::setIdentifier(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ClassDeclarationWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ClassDeclarationWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
