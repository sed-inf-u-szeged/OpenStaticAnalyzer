...
napi_ref ImportExpressionWrapper::constructor;

ImportExpressionWrapper::ImportExpressionWrapper(): env_(nullptr), wrapper_(nullptr) {}

ImportExpressionWrapper::~ImportExpressionWrapper(){ napi_delete_reference(env_, wrapper_); }

void ImportExpressionWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  ImportExpressionWrapper* obj = reinterpret_cast<ImportExpressionWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~ImportExpressionWrapper();
}
napi_value ImportExpressionWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  ...
  napi_value cons;
  status = napi_define_class(env, "ImportExpressionWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value ImportExpressionWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  ImportExpressionWrapper* obj = new ImportExpressionWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), ImportExpressionWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status ImportExpressionWrapper::NewInstance(napi_env env, expression::ImportExpression* arg, napi_value* instance) {
  ...
  ImportExpressionWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value ImportExpressionWrapper::setSource(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::expression::ImportExpression* source = dynamic_cast<columbus::javascript::asg::expression::ImportExpression*>(obj->_nativeObj);
  ...
  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast expression::ImportExpression" );
  }
  if(target == nullptr){
...
napi_value ImportExpressionWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ImportExpressionWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ImportExpressionWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
