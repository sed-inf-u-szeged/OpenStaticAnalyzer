...
napi_ref ObjectPatternWrapper::constructor;

ObjectPatternWrapper::ObjectPatternWrapper(): env_(nullptr), wrapper_(nullptr) {}

ObjectPatternWrapper::~ObjectPatternWrapper(){ napi_delete_reference(env_, wrapper_); }

void ObjectPatternWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  ObjectPatternWrapper* obj = reinterpret_cast<ObjectPatternWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~ObjectPatternWrapper();
}
napi_value ObjectPatternWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  ...
  napi_value cons;
  status = napi_define_class(env, "ObjectPatternWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value ObjectPatternWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  ObjectPatternWrapper* obj = new ObjectPatternWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), ObjectPatternWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status ObjectPatternWrapper::NewInstance(napi_env env, statement::ObjectPattern* arg, napi_value* instance) {
  ...
  ObjectPatternWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value ObjectPatternWrapper::addProperties(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::statement::ObjectPattern* source = dynamic_cast<columbus::javascript::asg::statement::ObjectPattern*>(obj->_nativeObj);
  columbus::javascript::asg::base::Positioned* target = dynamic_cast<columbus::javascript::asg::base::Positioned*>(param->_nativeObj);

  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast statement::ObjectPattern" );
  }
  if(target == nullptr){
...
napi_value ObjectPatternWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ObjectPatternWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ObjectPatternWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
