...
napi_ref ClassBodyWrapper::constructor;

ClassBodyWrapper::ClassBodyWrapper(): env_(nullptr), wrapper_(nullptr) {}

ClassBodyWrapper::~ClassBodyWrapper(){ napi_delete_reference(env_, wrapper_); }

void ClassBodyWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  ClassBodyWrapper* obj = reinterpret_cast<ClassBodyWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~ClassBodyWrapper();
}
napi_value ClassBodyWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  ...
  napi_value cons;
  status = napi_define_class(env, "ClassBodyWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value ClassBodyWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  ClassBodyWrapper* obj = new ClassBodyWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), ClassBodyWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status ClassBodyWrapper::NewInstance(napi_env env, structure::ClassBody* arg, napi_value* instance) {
  ...
  ClassBodyWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value ClassBodyWrapper::addBody(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ClassBodyWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ClassBodyWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ClassBodyWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
