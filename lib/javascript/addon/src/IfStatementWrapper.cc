...
napi_ref IfStatementWrapper::constructor;

IfStatementWrapper::IfStatementWrapper(): env_(nullptr), wrapper_(nullptr) {}

IfStatementWrapper::~IfStatementWrapper(){ napi_delete_reference(env_, wrapper_); }

void IfStatementWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  IfStatementWrapper* obj = reinterpret_cast<IfStatementWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~IfStatementWrapper();
}
napi_value IfStatementWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  ...
  napi_value cons;
  status = napi_define_class(env, "IfStatementWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value IfStatementWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  IfStatementWrapper* obj = new IfStatementWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), IfStatementWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status IfStatementWrapper::NewInstance(napi_env env, statement::IfStatement* arg, napi_value* instance) {
  ...
  IfStatementWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value IfStatementWrapper::setTest(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::statement::IfStatement* source = dynamic_cast<columbus::javascript::asg::statement::IfStatement*>(obj->_nativeObj);
  ...
  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast statement::IfStatement" );
  }
  if(target == nullptr){
...
napi_value IfStatementWrapper::setConsequent(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value IfStatementWrapper::setAlternate(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value IfStatementWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value IfStatementWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value IfStatementWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
