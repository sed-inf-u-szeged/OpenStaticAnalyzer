...
napi_ref NewExpressionWrapper::constructor;

NewExpressionWrapper::NewExpressionWrapper(): env_(nullptr), wrapper_(nullptr) {}

NewExpressionWrapper::~NewExpressionWrapper(){ napi_delete_reference(env_, wrapper_); }

void NewExpressionWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  NewExpressionWrapper* obj = reinterpret_cast<NewExpressionWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~NewExpressionWrapper();
}
napi_value NewExpressionWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  ...
  napi_value cons;
  status = napi_define_class(env, "NewExpressionWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value NewExpressionWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  NewExpressionWrapper* obj = new NewExpressionWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), NewExpressionWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status NewExpressionWrapper::NewInstance(napi_env env, expression::NewExpression* arg, napi_value* instance) {
  ...
  NewExpressionWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value NewExpressionWrapper::setCallee(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::expression::NewExpression* source = dynamic_cast<columbus::javascript::asg::expression::NewExpression*>(obj->_nativeObj);
  ...
  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast expression::NewExpression" );
  }
  if(target == nullptr){
...
napi_value NewExpressionWrapper::addArguments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value NewExpressionWrapper::addCalls(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value NewExpressionWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value NewExpressionWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value NewExpressionWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
