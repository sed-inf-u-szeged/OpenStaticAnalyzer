...
napi_ref ExportDefaultDeclarationWrapper::constructor;

ExportDefaultDeclarationWrapper::ExportDefaultDeclarationWrapper(): env_(nullptr), wrapper_(nullptr) {}

ExportDefaultDeclarationWrapper::~ExportDefaultDeclarationWrapper(){ napi_delete_reference(env_, wrapper_); }

void ExportDefaultDeclarationWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  ExportDefaultDeclarationWrapper* obj = reinterpret_cast<ExportDefaultDeclarationWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~ExportDefaultDeclarationWrapper();
}
napi_value ExportDefaultDeclarationWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  ...
  napi_value cons;
  status = napi_define_class(env, "ExportDefaultDeclarationWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value ExportDefaultDeclarationWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  ExportDefaultDeclarationWrapper* obj = new ExportDefaultDeclarationWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), ExportDefaultDeclarationWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status ExportDefaultDeclarationWrapper::NewInstance(napi_env env, declaration::ExportDefaultDeclaration* arg, napi_value* instance) {
  ...
  ExportDefaultDeclarationWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value ExportDefaultDeclarationWrapper::setDeclaration(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::declaration::ExportDefaultDeclaration* source = dynamic_cast<columbus::javascript::asg::declaration::ExportDefaultDeclaration*>(obj->_nativeObj);
  columbus::javascript::asg::base::Positioned* target = dynamic_cast<columbus::javascript::asg::base::Positioned*>(param->_nativeObj);

  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast declaration::ExportDefaultDeclaration" );
  }
  if(target == nullptr){
...
napi_value ExportDefaultDeclarationWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ExportDefaultDeclarationWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ExportDefaultDeclarationWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
