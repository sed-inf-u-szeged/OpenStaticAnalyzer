...
napi_ref BooleanLiteralWrapper::constructor;

BooleanLiteralWrapper::BooleanLiteralWrapper(): env_(nullptr), wrapper_(nullptr) {}

BooleanLiteralWrapper::~BooleanLiteralWrapper(){ napi_delete_reference(env_, wrapper_); }

void BooleanLiteralWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  BooleanLiteralWrapper* obj = reinterpret_cast<BooleanLiteralWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~BooleanLiteralWrapper();
}
napi_value BooleanLiteralWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  ...
  napi_value cons;
  status = napi_define_class(env, "BooleanLiteralWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value BooleanLiteralWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  BooleanLiteralWrapper* obj = new BooleanLiteralWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), BooleanLiteralWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status BooleanLiteralWrapper::NewInstance(napi_env env, expression::BooleanLiteral* arg, napi_value* instance) {
  ...
  BooleanLiteralWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value BooleanLiteralWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::expression::BooleanLiteral* source = dynamic_cast<columbus::javascript::asg::expression::BooleanLiteral*>(obj->_nativeObj);
  columbus::javascript::asg::base::Comment* target = dynamic_cast<columbus::javascript::asg::base::Comment*>(param->_nativeObj);

  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast expression::BooleanLiteral" );
  }
  if(target == nullptr){
...
napi_value BooleanLiteralWrapper::setValue(napi_env env, napi_callback_info info){ 
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  assert(status == napi_ok);
  dynamic_cast<columbus::javascript::asg::expression::BooleanLiteral*>(obj->_nativeObj)->setValue( b );
  return nullptr;
}
napi_value BooleanLiteralWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value BooleanLiteralWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
...
napi_value BooleanLiteralWrapper::setRaw(napi_env env, napi_callback_info info){ 
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
