...
napi_ref FunctionWrapper::constructor;

FunctionWrapper::FunctionWrapper(): env_(nullptr), wrapper_(nullptr) {}

FunctionWrapper::~FunctionWrapper(){ napi_delete_reference(env_, wrapper_); }

void FunctionWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  FunctionWrapper* obj = reinterpret_cast<FunctionWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~FunctionWrapper();
}
napi_value FunctionWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  ...
  napi_value cons;
  status = napi_define_class(env, "FunctionWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value FunctionWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  FunctionWrapper* obj = new FunctionWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), FunctionWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status FunctionWrapper::NewInstance(napi_env env, statement::Function* arg, napi_value* instance) {
...
napi_value FunctionWrapper::addParams(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::statement::Function* source = dynamic_cast<columbus::javascript::asg::statement::Function*>(obj->_nativeObj);
...
napi_value FunctionWrapper::setBody(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value FunctionWrapper::setIdentifier(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value FunctionWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value FunctionWrapper::setGenerator(napi_env env, napi_callback_info info){ 
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value FunctionWrapper::setAsync(napi_env env, napi_callback_info info){ 
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value FunctionWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value FunctionWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
