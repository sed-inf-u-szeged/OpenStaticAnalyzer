...
napi_ref ThisExpressionWrapper::constructor;

ThisExpressionWrapper::ThisExpressionWrapper(): env_(nullptr), wrapper_(nullptr) {}

ThisExpressionWrapper::~ThisExpressionWrapper(){ napi_delete_reference(env_, wrapper_); }

void ThisExpressionWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  ThisExpressionWrapper* obj = reinterpret_cast<ThisExpressionWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~ThisExpressionWrapper();
}
napi_value ThisExpressionWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  ...
  napi_value cons;
  status = napi_define_class(env, "ThisExpressionWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value ThisExpressionWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  ThisExpressionWrapper* obj = new ThisExpressionWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), ThisExpressionWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status ThisExpressionWrapper::NewInstance(napi_env env, expression::ThisExpression* arg, napi_value* instance) {
  ...
  ThisExpressionWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value ThisExpressionWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::expression::ThisExpression* source = dynamic_cast<columbus::javascript::asg::expression::ThisExpression*>(obj->_nativeObj);
  columbus::javascript::asg::base::Comment* target = dynamic_cast<columbus::javascript::asg::base::Comment*>(param->_nativeObj);

  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast expression::ThisExpression" );
  }
  if(target == nullptr){
...
napi_value ThisExpressionWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ThisExpressionWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
