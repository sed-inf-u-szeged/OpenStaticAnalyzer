...
napi_ref TemplateLiteralWrapper::constructor;

TemplateLiteralWrapper::TemplateLiteralWrapper(): env_(nullptr), wrapper_(nullptr) {}

TemplateLiteralWrapper::~TemplateLiteralWrapper(){ napi_delete_reference(env_, wrapper_); }

void TemplateLiteralWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  TemplateLiteralWrapper* obj = reinterpret_cast<TemplateLiteralWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~TemplateLiteralWrapper();
}
napi_value TemplateLiteralWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  napi_property_descriptor props [] = {
  DECLARE_NAPI_METHOD( "addQuasis", addQuasis),
  DECLARE_NAPI_METHOD( "addExpressions", addExpressions),
  ...
  napi_value cons;
  status = napi_define_class(env, "TemplateLiteralWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value TemplateLiteralWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  TemplateLiteralWrapper* obj = new TemplateLiteralWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), TemplateLiteralWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status TemplateLiteralWrapper::NewInstance(napi_env env, expression::TemplateLiteral* arg, napi_value* instance) {
  ...
  TemplateLiteralWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value TemplateLiteralWrapper::addQuasis(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::expression::TemplateLiteral* source = dynamic_cast<columbus::javascript::asg::expression::TemplateLiteral*>(obj->_nativeObj);
  columbus::javascript::asg::expression::TemplateElement* target = dynamic_cast<columbus::javascript::asg::expression::TemplateElement*>(param->_nativeObj);
  ...
  source->addQuasis(target);
  return nullptr;
}
napi_value TemplateLiteralWrapper::addExpressions(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  source->addExpressions(target);
  return nullptr;
}
napi_value TemplateLiteralWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value TemplateLiteralWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value TemplateLiteralWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
