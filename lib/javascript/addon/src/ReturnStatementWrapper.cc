...
napi_ref ReturnStatementWrapper::constructor;

ReturnStatementWrapper::ReturnStatementWrapper(): env_(nullptr), wrapper_(nullptr) {}

ReturnStatementWrapper::~ReturnStatementWrapper(){ napi_delete_reference(env_, wrapper_); }

void ReturnStatementWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  ReturnStatementWrapper* obj = reinterpret_cast<ReturnStatementWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~ReturnStatementWrapper();
}
napi_value ReturnStatementWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  ...
  napi_value cons;
  status = napi_define_class(env, "ReturnStatementWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value ReturnStatementWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  ReturnStatementWrapper* obj = new ReturnStatementWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), ReturnStatementWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status ReturnStatementWrapper::NewInstance(napi_env env, statement::ReturnStatement* arg, napi_value* instance) {
  ...
  ReturnStatementWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value ReturnStatementWrapper::setArgument(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::statement::ReturnStatement* source = dynamic_cast<columbus::javascript::asg::statement::ReturnStatement*>(obj->_nativeObj);
  ...
  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast statement::ReturnStatement" );
  }
  if(target == nullptr){
...
napi_value ReturnStatementWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ReturnStatementWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ReturnStatementWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
