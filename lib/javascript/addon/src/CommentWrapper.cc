...
napi_ref CommentWrapper::constructor;

CommentWrapper::CommentWrapper(): env_(nullptr), wrapper_(nullptr) {}

CommentWrapper::~CommentWrapper(){ napi_delete_reference(env_, wrapper_); }

void CommentWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  CommentWrapper* obj = reinterpret_cast<CommentWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~CommentWrapper();
}
napi_value CommentWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  napi_property_descriptor props [] = {
  DECLARE_NAPI_METHOD( "addComments", addComments),
    DECLARE_NAPI_METHOD("setText", setText),
    DECLARE_NAPI_METHOD("setType", setType),
    DECLARE_NAPI_METHOD("setLocation", setLocation),
    DECLARE_NAPI_METHOD("setPath", setPath),
    DECLARE_NAPI_METHOD("setPosition", setPosition),
  };

  napi_value cons;
  status = napi_define_class(env, "CommentWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value CommentWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  CommentWrapper* obj = new CommentWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), CommentWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status CommentWrapper::NewInstance(napi_env env, base::Comment* arg, napi_value* instance) {
...
napi_value CommentWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::base::Comment* source = dynamic_cast<columbus::javascript::asg::base::Comment*>(obj->_nativeObj);
...
napi_value CommentWrapper::setText(napi_env env, napi_callback_info info){ 
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  if(paramtype == napi_null){
    dynamic_cast<columbus::javascript::asg::base::Comment*>(obj->_nativeObj)->setText( std::string("null") );
  }
  else{
    char buffer[1024];
    ...
    dynamic_cast<columbus::javascript::asg::base::Comment*>(obj->_nativeObj)->setText( param );
  }
  return nullptr;
}
napi_value CommentWrapper::setType(napi_env env, napi_callback_info info){ 
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  if( param == "ctLine" ){
    dynamic_cast<columbus::javascript::asg::base::Comment*>(obj->_nativeObj)->setType( ctLine );
  }
  if( param == "ctBlock" ){
    dynamic_cast<columbus::javascript::asg::base::Comment*>(obj->_nativeObj)->setType( ctBlock );
  }
  return nullptr;
}
napi_value CommentWrapper::setLocation(napi_env env, napi_callback_info info){ 
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  if( param == "clTrailing" ){
    dynamic_cast<columbus::javascript::asg::base::Comment*>(obj->_nativeObj)->setLocation( clTrailing );
  }
  if( param == "clLeading" ){
    dynamic_cast<columbus::javascript::asg::base::Comment*>(obj->_nativeObj)->setLocation( clLeading );
  }
  return nullptr;
}
napi_value CommentWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value CommentWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
