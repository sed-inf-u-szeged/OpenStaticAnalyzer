...
napi_ref SwitchCaseWrapper::constructor;

SwitchCaseWrapper::SwitchCaseWrapper(): env_(nullptr), wrapper_(nullptr) {}

SwitchCaseWrapper::~SwitchCaseWrapper(){ napi_delete_reference(env_, wrapper_); }

void SwitchCaseWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  SwitchCaseWrapper* obj = reinterpret_cast<SwitchCaseWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~SwitchCaseWrapper();
}
napi_value SwitchCaseWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  napi_property_descriptor props [] = {
  DECLARE_NAPI_METHOD( "setTest", setTest),
  DECLARE_NAPI_METHOD( "addConsequent", addConsequent),
  ...
  napi_value cons;
  status = napi_define_class(env, "SwitchCaseWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value SwitchCaseWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  SwitchCaseWrapper* obj = new SwitchCaseWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), SwitchCaseWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status SwitchCaseWrapper::NewInstance(napi_env env, statement::SwitchCase* arg, napi_value* instance) {
  ...
  SwitchCaseWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value SwitchCaseWrapper::setTest(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::statement::SwitchCase* source = dynamic_cast<columbus::javascript::asg::statement::SwitchCase*>(obj->_nativeObj);
...
napi_value SwitchCaseWrapper::addConsequent(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  source->addConsequent(target);
  return nullptr;
}
napi_value SwitchCaseWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value SwitchCaseWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value SwitchCaseWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
