...
napi_ref ArrayExpressionWrapper::constructor;

ArrayExpressionWrapper::ArrayExpressionWrapper(): env_(nullptr), wrapper_(nullptr) {}

ArrayExpressionWrapper::~ArrayExpressionWrapper(){ napi_delete_reference(env_, wrapper_); }

void ArrayExpressionWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  ArrayExpressionWrapper* obj = reinterpret_cast<ArrayExpressionWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~ArrayExpressionWrapper();
}
napi_value ArrayExpressionWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  napi_property_descriptor props [] = {
  DECLARE_NAPI_METHOD( "addElements", addElements),
  ...
  napi_value cons;
  status = napi_define_class(env, "ArrayExpressionWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value ArrayExpressionWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  ArrayExpressionWrapper* obj = new ArrayExpressionWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), ArrayExpressionWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status ArrayExpressionWrapper::NewInstance(napi_env env, expression::ArrayExpression* arg, napi_value* instance) {
  ...
  ArrayExpressionWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value ArrayExpressionWrapper::addElements(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::expression::ArrayExpression* source = dynamic_cast<columbus::javascript::asg::expression::ArrayExpression*>(obj->_nativeObj);
  columbus::javascript::asg::base::Positioned* target = dynamic_cast<columbus::javascript::asg::base::Positioned*>(param->_nativeObj);

  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast expression::ArrayExpression" );
  }
  if(target == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast base::Positioned" );
  }

  source->addElements(target);
  return nullptr;
}
napi_value ArrayExpressionWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ArrayExpressionWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ArrayExpressionWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
