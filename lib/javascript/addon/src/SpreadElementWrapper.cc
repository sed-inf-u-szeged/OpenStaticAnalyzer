...
napi_ref SpreadElementWrapper::constructor;

SpreadElementWrapper::SpreadElementWrapper(): env_(nullptr), wrapper_(nullptr) {}

SpreadElementWrapper::~SpreadElementWrapper(){ napi_delete_reference(env_, wrapper_); }

void SpreadElementWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  SpreadElementWrapper* obj = reinterpret_cast<SpreadElementWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~SpreadElementWrapper();
}
napi_value SpreadElementWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  ...
  napi_value cons;
  status = napi_define_class(env, "SpreadElementWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value SpreadElementWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  SpreadElementWrapper* obj = new SpreadElementWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), SpreadElementWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status SpreadElementWrapper::NewInstance(napi_env env, expression::SpreadElement* arg, napi_value* instance) {
  ...
  SpreadElementWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value SpreadElementWrapper::setArgument(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::expression::SpreadElement* source = dynamic_cast<columbus::javascript::asg::expression::SpreadElement*>(obj->_nativeObj);
  ...
  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast expression::SpreadElement" );
  }
  if(target == nullptr){
...
napi_value SpreadElementWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value SpreadElementWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value SpreadElementWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
