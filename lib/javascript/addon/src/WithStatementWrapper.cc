...
napi_ref WithStatementWrapper::constructor;

WithStatementWrapper::WithStatementWrapper(): env_(nullptr), wrapper_(nullptr) {}

WithStatementWrapper::~WithStatementWrapper(){ napi_delete_reference(env_, wrapper_); }

void WithStatementWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  WithStatementWrapper* obj = reinterpret_cast<WithStatementWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~WithStatementWrapper();
}
napi_value WithStatementWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  ...
  napi_value cons;
  status = napi_define_class(env, "WithStatementWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value WithStatementWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  WithStatementWrapper* obj = new WithStatementWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), WithStatementWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status WithStatementWrapper::NewInstance(napi_env env, statement::WithStatement* arg, napi_value* instance) {
  ...
  WithStatementWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value WithStatementWrapper::setObject(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::statement::WithStatement* source = dynamic_cast<columbus::javascript::asg::statement::WithStatement*>(obj->_nativeObj);
  ...
  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast statement::WithStatement" );
  }
  if(target == nullptr){
...
napi_value WithStatementWrapper::setBody(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value WithStatementWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value WithStatementWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value WithStatementWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
