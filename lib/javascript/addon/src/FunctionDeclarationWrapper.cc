...
napi_ref FunctionDeclarationWrapper::constructor;

FunctionDeclarationWrapper::FunctionDeclarationWrapper(): env_(nullptr), wrapper_(nullptr) {}

FunctionDeclarationWrapper::~FunctionDeclarationWrapper(){ napi_delete_reference(env_, wrapper_); }

void FunctionDeclarationWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  FunctionDeclarationWrapper* obj = reinterpret_cast<FunctionDeclarationWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~FunctionDeclarationWrapper();
}
napi_value FunctionDeclarationWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  ...
  napi_value cons;
  status = napi_define_class(env, "FunctionDeclarationWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value FunctionDeclarationWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  FunctionDeclarationWrapper* obj = new FunctionDeclarationWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), FunctionDeclarationWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status FunctionDeclarationWrapper::NewInstance(napi_env env, declaration::FunctionDeclaration* arg, napi_value* instance) {
  ...
  FunctionDeclarationWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value FunctionDeclarationWrapper::addParams(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::declaration::FunctionDeclaration* source = dynamic_cast<columbus::javascript::asg::declaration::FunctionDeclaration*>(obj->_nativeObj);
  columbus::javascript::asg::statement::Pattern* target = dynamic_cast<columbus::javascript::asg::statement::Pattern*>(param->_nativeObj);

  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast declaration::FunctionDeclaration" );
  }
  if(target == nullptr){
...
napi_value FunctionDeclarationWrapper::setBody(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value FunctionDeclarationWrapper::setIdentifier(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value FunctionDeclarationWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value FunctionDeclarationWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value FunctionDeclarationWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
...
napi_value FunctionDeclarationWrapper::setGenerator(napi_env env, napi_callback_info info){ 
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value FunctionDeclarationWrapper::setAsync(napi_env env, napi_callback_info info){ 
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
