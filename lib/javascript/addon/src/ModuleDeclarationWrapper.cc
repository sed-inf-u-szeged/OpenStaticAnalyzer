...
napi_ref ModuleDeclarationWrapper::constructor;

ModuleDeclarationWrapper::ModuleDeclarationWrapper(): env_(nullptr), wrapper_(nullptr) {}

ModuleDeclarationWrapper::~ModuleDeclarationWrapper(){ napi_delete_reference(env_, wrapper_); }

void ModuleDeclarationWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  ModuleDeclarationWrapper* obj = reinterpret_cast<ModuleDeclarationWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~ModuleDeclarationWrapper();
}
napi_value ModuleDeclarationWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  ...
  napi_value cons;
  status = napi_define_class(env, "ModuleDeclarationWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value ModuleDeclarationWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  ModuleDeclarationWrapper* obj = new ModuleDeclarationWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), ModuleDeclarationWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status ModuleDeclarationWrapper::NewInstance(napi_env env, declaration::ModuleDeclaration* arg, napi_value* instance) {
  ...
  ModuleDeclarationWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value ModuleDeclarationWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::declaration::ModuleDeclaration* source = dynamic_cast<columbus::javascript::asg::declaration::ModuleDeclaration*>(obj->_nativeObj);
  columbus::javascript::asg::base::Comment* target = dynamic_cast<columbus::javascript::asg::base::Comment*>(param->_nativeObj);

  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast declaration::ModuleDeclaration" );
  }
  if(target == nullptr){
...
napi_value ModuleDeclarationWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ModuleDeclarationWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
