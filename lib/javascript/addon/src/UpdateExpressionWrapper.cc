...
napi_ref UpdateExpressionWrapper::constructor;

UpdateExpressionWrapper::UpdateExpressionWrapper(): env_(nullptr), wrapper_(nullptr) {}

UpdateExpressionWrapper::~UpdateExpressionWrapper(){ napi_delete_reference(env_, wrapper_); }

void UpdateExpressionWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  UpdateExpressionWrapper* obj = reinterpret_cast<UpdateExpressionWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~UpdateExpressionWrapper();
}
napi_value UpdateExpressionWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  ...
  napi_value cons;
  status = napi_define_class(env, "UpdateExpressionWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value UpdateExpressionWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  UpdateExpressionWrapper* obj = new UpdateExpressionWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), UpdateExpressionWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status UpdateExpressionWrapper::NewInstance(napi_env env, expression::UpdateExpression* arg, napi_value* instance) {
  ...
  UpdateExpressionWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value UpdateExpressionWrapper::setArgument(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::expression::UpdateExpression* source = dynamic_cast<columbus::javascript::asg::expression::UpdateExpression*>(obj->_nativeObj);
  ...
  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast expression::UpdateExpression" );
  }
  if(target == nullptr){
...
napi_value UpdateExpressionWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value UpdateExpressionWrapper::setOperator(napi_env env, napi_callback_info info){ 
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  if( param == "upoIncrement" ){
    dynamic_cast<columbus::javascript::asg::expression::UpdateExpression*>(obj->_nativeObj)->setOperator( upoIncrement );
  }
  if( param == "upoDecrement" ){
    dynamic_cast<columbus::javascript::asg::expression::UpdateExpression*>(obj->_nativeObj)->setOperator( upoDecrement );
  }
  return nullptr;
}
napi_value UpdateExpressionWrapper::setPrefix(napi_env env, napi_callback_info info){ 
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  assert(status == napi_ok);
  dynamic_cast<columbus::javascript::asg::expression::UpdateExpression*>(obj->_nativeObj)->setPrefix( b );
  return nullptr;
}
napi_value UpdateExpressionWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value UpdateExpressionWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
