...
napi_ref ExpressionStatementWrapper::constructor;

ExpressionStatementWrapper::ExpressionStatementWrapper(): env_(nullptr), wrapper_(nullptr) {}

ExpressionStatementWrapper::~ExpressionStatementWrapper(){ napi_delete_reference(env_, wrapper_); }

void ExpressionStatementWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  ExpressionStatementWrapper* obj = reinterpret_cast<ExpressionStatementWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~ExpressionStatementWrapper();
}
napi_value ExpressionStatementWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  ...
  napi_value cons;
  status = napi_define_class(env, "ExpressionStatementWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value ExpressionStatementWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  ExpressionStatementWrapper* obj = new ExpressionStatementWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), ExpressionStatementWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status ExpressionStatementWrapper::NewInstance(napi_env env, statement::ExpressionStatement* arg, napi_value* instance) {
  ...
  ExpressionStatementWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value ExpressionStatementWrapper::setExpression(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::statement::ExpressionStatement* source = dynamic_cast<columbus::javascript::asg::statement::ExpressionStatement*>(obj->_nativeObj);
  ...
  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast statement::ExpressionStatement" );
  }
  if(target == nullptr){
...
napi_value ExpressionStatementWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ExpressionStatementWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ExpressionStatementWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
