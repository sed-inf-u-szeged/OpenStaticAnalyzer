...
napi_ref PrivateIdentifierWrapper::constructor;

PrivateIdentifierWrapper::PrivateIdentifierWrapper(): env_(nullptr), wrapper_(nullptr) {}

PrivateIdentifierWrapper::~PrivateIdentifierWrapper(){ napi_delete_reference(env_, wrapper_); }

void PrivateIdentifierWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  PrivateIdentifierWrapper* obj = reinterpret_cast<PrivateIdentifierWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~PrivateIdentifierWrapper();
}
napi_value PrivateIdentifierWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  ...
  DECLARE_NAPI_METHOD( "addComments", addComments),
    ...
    DECLARE_NAPI_METHOD("setName", setName),
  };

  napi_value cons;
  status = napi_define_class(env, "PrivateIdentifierWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value PrivateIdentifierWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  PrivateIdentifierWrapper* obj = new PrivateIdentifierWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), PrivateIdentifierWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status PrivateIdentifierWrapper::NewInstance(napi_env env, expression::PrivateIdentifier* arg, napi_value* instance) {
  ...
  PrivateIdentifierWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value PrivateIdentifierWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::expression::PrivateIdentifier* source = dynamic_cast<columbus::javascript::asg::expression::PrivateIdentifier*>(obj->_nativeObj);
  columbus::javascript::asg::base::Comment* target = dynamic_cast<columbus::javascript::asg::base::Comment*>(param->_nativeObj);

  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast expression::PrivateIdentifier" );
  }
  if(target == nullptr){
...
napi_value PrivateIdentifierWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value PrivateIdentifierWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
...
napi_value PrivateIdentifierWrapper::setName(napi_env env, napi_callback_info info){ 
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  if(paramtype == napi_null){
    dynamic_cast<columbus::javascript::asg::base::Named*>(obj->_nativeObj)->setName( std::string("null") );
  }
  else{
    char buffer[1024];
    size_t buffer_size = 1024, result_size = 0;
    ...
    dynamic_cast<columbus::javascript::asg::base::Named*>(obj->_nativeObj)->setName( param );
  }
  return nullptr;
}
}}}} //end of namespaces