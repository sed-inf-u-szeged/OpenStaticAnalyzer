...
napi_ref YieldExpressionWrapper::constructor;

YieldExpressionWrapper::YieldExpressionWrapper(): env_(nullptr), wrapper_(nullptr) {}

YieldExpressionWrapper::~YieldExpressionWrapper(){ napi_delete_reference(env_, wrapper_); }

void YieldExpressionWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  YieldExpressionWrapper* obj = reinterpret_cast<YieldExpressionWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~YieldExpressionWrapper();
}
napi_value YieldExpressionWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  napi_property_descriptor props [] = {
  DECLARE_NAPI_METHOD( "setArgument", setArgument),
  DECLARE_NAPI_METHOD( "addComments", addComments),
    DECLARE_NAPI_METHOD("setDelegate", setDelegate),
    DECLARE_NAPI_METHOD("setPath", setPath),
    DECLARE_NAPI_METHOD("setPosition", setPosition),
  };

  napi_value cons;
  status = napi_define_class(env, "YieldExpressionWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value YieldExpressionWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  YieldExpressionWrapper* obj = new YieldExpressionWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), YieldExpressionWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status YieldExpressionWrapper::NewInstance(napi_env env, expression::YieldExpression* arg, napi_value* instance) {
  ...
  YieldExpressionWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value YieldExpressionWrapper::setArgument(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::expression::YieldExpression* source = dynamic_cast<columbus::javascript::asg::expression::YieldExpression*>(obj->_nativeObj);
  ...
  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast expression::YieldExpression" );
  }
  if(target == nullptr){
...
napi_value YieldExpressionWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value YieldExpressionWrapper::setDelegate(napi_env env, napi_callback_info info){ 
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  assert(status == napi_ok);
  dynamic_cast<columbus::javascript::asg::expression::YieldExpression*>(obj->_nativeObj)->setDelegate( b );
  return nullptr;
}
napi_value YieldExpressionWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value YieldExpressionWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
