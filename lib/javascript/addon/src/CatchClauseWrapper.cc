...
napi_ref CatchClauseWrapper::constructor;

CatchClauseWrapper::CatchClauseWrapper(): env_(nullptr), wrapper_(nullptr) {}

CatchClauseWrapper::~CatchClauseWrapper(){ napi_delete_reference(env_, wrapper_); }

void CatchClauseWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  CatchClauseWrapper* obj = reinterpret_cast<CatchClauseWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~CatchClauseWrapper();
}
napi_value CatchClauseWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  napi_property_descriptor props [] = {
  DECLARE_NAPI_METHOD( "setParam", setParam),
  ...
  napi_value cons;
  status = napi_define_class(env, "CatchClauseWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value CatchClauseWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  CatchClauseWrapper* obj = new CatchClauseWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), CatchClauseWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status CatchClauseWrapper::NewInstance(napi_env env, statement::CatchClause* arg, napi_value* instance) {
  ...
  CatchClauseWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value CatchClauseWrapper::setParam(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::statement::CatchClause* source = dynamic_cast<columbus::javascript::asg::statement::CatchClause*>(obj->_nativeObj);
  ...
  source->setParam(target);
  return nullptr;
}
napi_value CatchClauseWrapper::setBody(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value CatchClauseWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value CatchClauseWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value CatchClauseWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
