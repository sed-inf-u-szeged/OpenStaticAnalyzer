...
napi_ref ForOfStatementWrapper::constructor;

ForOfStatementWrapper::ForOfStatementWrapper(): env_(nullptr), wrapper_(nullptr) {}

ForOfStatementWrapper::~ForOfStatementWrapper(){ napi_delete_reference(env_, wrapper_); }

void ForOfStatementWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  ForOfStatementWrapper* obj = reinterpret_cast<ForOfStatementWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~ForOfStatementWrapper();
}
napi_value ForOfStatementWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  ...
  napi_value cons;
  status = napi_define_class(env, "ForOfStatementWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value ForOfStatementWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  ForOfStatementWrapper* obj = new ForOfStatementWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), ForOfStatementWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status ForOfStatementWrapper::NewInstance(napi_env env, statement::ForOfStatement* arg, napi_value* instance) {
  ...
  ForOfStatementWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value ForOfStatementWrapper::setRight(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::statement::ForOfStatement* source = dynamic_cast<columbus::javascript::asg::statement::ForOfStatement*>(obj->_nativeObj);
  ...
  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast statement::ForOfStatement" );
  }
  if(target == nullptr){
...
napi_value ForOfStatementWrapper::setBody(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ForOfStatementWrapper::setLeft(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ForOfStatementWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ForOfStatementWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
