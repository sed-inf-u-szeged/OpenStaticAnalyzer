...
napi_ref TaggedTemplateExpressionWrapper::constructor;

TaggedTemplateExpressionWrapper::TaggedTemplateExpressionWrapper(): env_(nullptr), wrapper_(nullptr) {}

TaggedTemplateExpressionWrapper::~TaggedTemplateExpressionWrapper(){ napi_delete_reference(env_, wrapper_); }

void TaggedTemplateExpressionWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  TaggedTemplateExpressionWrapper* obj = reinterpret_cast<TaggedTemplateExpressionWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~TaggedTemplateExpressionWrapper();
}
napi_value TaggedTemplateExpressionWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  napi_property_descriptor props [] = {
  DECLARE_NAPI_METHOD( "setTag", setTag),
  DECLARE_NAPI_METHOD( "setQuasi", setQuasi),
  ...
  napi_value cons;
  status = napi_define_class(env, "TaggedTemplateExpressionWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value TaggedTemplateExpressionWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  TaggedTemplateExpressionWrapper* obj = new TaggedTemplateExpressionWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), TaggedTemplateExpressionWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status TaggedTemplateExpressionWrapper::NewInstance(napi_env env, expression::TaggedTemplateExpression* arg, napi_value* instance) {
  ...
  TaggedTemplateExpressionWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value TaggedTemplateExpressionWrapper::setTag(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::expression::TaggedTemplateExpression* source = dynamic_cast<columbus::javascript::asg::expression::TaggedTemplateExpression*>(obj->_nativeObj);
  ...
  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast expression::TaggedTemplateExpression" );
  }
  if(target == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast expression::Expression" );
  }

  source->setTag(target);
  return nullptr;
}
napi_value TaggedTemplateExpressionWrapper::setQuasi(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::expression::TaggedTemplateExpression* source = dynamic_cast<columbus::javascript::asg::expression::TaggedTemplateExpression*>(obj->_nativeObj);
  columbus::javascript::asg::expression::TemplateLiteral* target = dynamic_cast<columbus::javascript::asg::expression::TemplateLiteral*>(param->_nativeObj);

  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast expression::TaggedTemplateExpression" );
  }
  if(target == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast expression::TemplateLiteral" );
  }

  source->setQuasi(target);
  return nullptr;
}
napi_value TaggedTemplateExpressionWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value TaggedTemplateExpressionWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value TaggedTemplateExpressionWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
