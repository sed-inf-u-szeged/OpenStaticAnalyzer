...
napi_ref RestElementWrapper::constructor;

RestElementWrapper::RestElementWrapper(): env_(nullptr), wrapper_(nullptr) {}

RestElementWrapper::~RestElementWrapper(){ napi_delete_reference(env_, wrapper_); }

void RestElementWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  RestElementWrapper* obj = reinterpret_cast<RestElementWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~RestElementWrapper();
}
napi_value RestElementWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  ...
  napi_value cons;
  status = napi_define_class(env, "RestElementWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value RestElementWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  RestElementWrapper* obj = new RestElementWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), RestElementWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status RestElementWrapper::NewInstance(napi_env env, statement::RestElement* arg, napi_value* instance) {
  ...
  RestElementWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value RestElementWrapper::setArgument(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::statement::RestElement* source = dynamic_cast<columbus::javascript::asg::statement::RestElement*>(obj->_nativeObj);
  columbus::javascript::asg::statement::Pattern* target = dynamic_cast<columbus::javascript::asg::statement::Pattern*>(param->_nativeObj);

  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast statement::RestElement" );
  }
  if(target == nullptr){
...
napi_value RestElementWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value RestElementWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value RestElementWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
