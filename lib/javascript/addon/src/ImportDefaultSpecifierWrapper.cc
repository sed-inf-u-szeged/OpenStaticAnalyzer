...
napi_ref ImportDefaultSpecifierWrapper::constructor;

ImportDefaultSpecifierWrapper::ImportDefaultSpecifierWrapper(): env_(nullptr), wrapper_(nullptr) {}

ImportDefaultSpecifierWrapper::~ImportDefaultSpecifierWrapper(){ napi_delete_reference(env_, wrapper_); }

void ImportDefaultSpecifierWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  ImportDefaultSpecifierWrapper* obj = reinterpret_cast<ImportDefaultSpecifierWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~ImportDefaultSpecifierWrapper();
}
napi_value ImportDefaultSpecifierWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  ...
  napi_value cons;
  status = napi_define_class(env, "ImportDefaultSpecifierWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value ImportDefaultSpecifierWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  ImportDefaultSpecifierWrapper* obj = new ImportDefaultSpecifierWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), ImportDefaultSpecifierWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status ImportDefaultSpecifierWrapper::NewInstance(napi_env env, structure::ImportDefaultSpecifier* arg, napi_value* instance) {
  ...
  ImportDefaultSpecifierWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value ImportDefaultSpecifierWrapper::setLocal(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::structure::ImportDefaultSpecifier* source = dynamic_cast<columbus::javascript::asg::structure::ImportDefaultSpecifier*>(obj->_nativeObj);
  ...
  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast structure::ImportDefaultSpecifier" );
  }
  if(target == nullptr){
...
napi_value ImportDefaultSpecifierWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ImportDefaultSpecifierWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ImportDefaultSpecifierWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
