...
napi_ref BreakStatementWrapper::constructor;

BreakStatementWrapper::BreakStatementWrapper(): env_(nullptr), wrapper_(nullptr) {}

BreakStatementWrapper::~BreakStatementWrapper(){ napi_delete_reference(env_, wrapper_); }

void BreakStatementWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  BreakStatementWrapper* obj = reinterpret_cast<BreakStatementWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~BreakStatementWrapper();
}
napi_value BreakStatementWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  ...
  napi_value cons;
  status = napi_define_class(env, "BreakStatementWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value BreakStatementWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  BreakStatementWrapper* obj = new BreakStatementWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), BreakStatementWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status BreakStatementWrapper::NewInstance(napi_env env, statement::BreakStatement* arg, napi_value* instance) {
  ...
  BreakStatementWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value BreakStatementWrapper::setLabel(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::statement::BreakStatement* source = dynamic_cast<columbus::javascript::asg::statement::BreakStatement*>(obj->_nativeObj);
  ...
  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast statement::BreakStatement" );
  }
  if(target == nullptr){
...
napi_value BreakStatementWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value BreakStatementWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value BreakStatementWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
