...
napi_ref ForStatementWrapper::constructor;

ForStatementWrapper::ForStatementWrapper(): env_(nullptr), wrapper_(nullptr) {}

ForStatementWrapper::~ForStatementWrapper(){ napi_delete_reference(env_, wrapper_); }

void ForStatementWrapper::Destructor(napi_env env, void* nativeObject, void* ){
  ForStatementWrapper* obj = reinterpret_cast<ForStatementWrapper*>(nativeObject);
  //delete obj->_nativeObj;
  obj->~ForStatementWrapper();
}
napi_value ForStatementWrapper::Init(napi_env env, napi_value& exports) {
  napi_status status;
  napi_property_descriptor props [] = {
  DECLARE_NAPI_METHOD( "setBody", setBody),
  DECLARE_NAPI_METHOD( "setTest", setTest),
  DECLARE_NAPI_METHOD( "setUpdate", setUpdate),
  ...
  napi_value cons;
  status = napi_define_class(env, "ForStatementWrapper", NAPI_AUTO_LENGTH, New, nullptr, sizeof(props) / sizeof(*props), props, &cons );
...
napi_value ForStatementWrapper::New(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  ...
  ForStatementWrapper* obj = new ForStatementWrapper();
  obj->env_ = env;
  status = napi_wrap(env, jsthis, reinterpret_cast<void*>(obj), ForStatementWrapper::Destructor, nullptr, &obj->wrapper_);
  assert(status == napi_ok);
...
napi_status ForStatementWrapper::NewInstance(napi_env env, statement::ForStatement* arg, napi_value* instance) {
  ...
  ForStatementWrapper* obj;
  status = napi_unwrap(env, *instance, reinterpret_cast<void**>(&obj));
  obj->_nativeObj = arg;
  return napi_ok;
}

napi_value ForStatementWrapper::setBody(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  columbus::javascript::asg::statement::ForStatement* source = dynamic_cast<columbus::javascript::asg::statement::ForStatement*>(obj->_nativeObj);
  ...
  if(source == nullptr){
    status = napi_throw_error(env, nullptr, "Cannot cast statement::ForStatement" );
  }
  if(target == nullptr){
...
napi_value ForStatementWrapper::setTest(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ForStatementWrapper::setUpdate(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
  ...
  source->setUpdate(target);
  return nullptr;
}
napi_value ForStatementWrapper::setInit(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ForStatementWrapper::addComments(napi_env env, napi_callback_info info){
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ForStatementWrapper::setPath(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 1;
  napi_value args[1];
...
napi_value ForStatementWrapper::setPosition(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value jsthis;
  size_t argc = 8;
  napi_value args[8];
