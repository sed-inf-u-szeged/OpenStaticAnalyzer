...
namespace logical { 
  MethodGenericInstance::MethodGenericInstance(NodeId _id, Factory *_factory) :
    Method(_id, _factory),
  ...
  MethodGenericInstance::~MethodGenericInstance() {
  }

  void MethodGenericInstance::clone(const MethodGenericInstance& other, bool tryOnVirtualParent) {
  ...
  void MethodGenericInstance::prepareDelete(bool tryOnVirtualParent){
    while (!hasArgumentsContainer.empty()) {
      const NodeId id = *hasArgumentsContainer.begin();
      if (factory->getExistsReverseEdges())
        factory->reverseEdges->removeEdge(id, this->getId(), edkMethodGenericInstance_HasArguments);
      hasArgumentsContainer.pop_front();
    }
    logical::Method::prepareDelete(false);
  }

  NodeKind MethodGenericInstance::getNodeKind() const {
    return ndkMethodGenericInstance;
  }

  bool MethodGenericInstance::getIsRealInstance() const {
    return m_isRealInstance;
  }

  void MethodGenericInstance::setIsRealInstance(bool _isRealInstance) {
    m_isRealInstance = _isRealInstance;
  }

  ListIteratorAssocTypeArgumentConstraintKind<type::Type> MethodGenericInstance::getArgumentsListIteratorAssocBegin() const {
    return ListIteratorAssocTypeArgumentConstraintKind<type::Type>(&hasArgumentsContainer, factory, true);
  }

  ListIteratorAssocTypeArgumentConstraintKind<type::Type> MethodGenericInstance::getArgumentsListIteratorAssocEnd() const {
    return ListIteratorAssocTypeArgumentConstraintKind<type::Type>(&hasArgumentsContainer, factory, false);
  }

  bool MethodGenericInstance::getArgumentsIsEmpty() const {
    return getArgumentsListIteratorAssocBegin() == getArgumentsListIteratorAssocEnd();
  }

  unsigned int MethodGenericInstance::getArgumentsSize() const {
    unsigned int size = 0;
  ...
  bool MethodGenericInstance::setEdge(EdgeKind edgeKind, NodeId edgeEnd, void* acValue, bool tryOnVirtualParent) {
    switch (edgeKind) {
  ...
  bool MethodGenericInstance::removeEdge(EdgeKind edgeKind, NodeId edgeEnd, bool tryOnVirtualParent) {
    switch (edgeKind) {
      case edkMethodGenericInstance_HasArguments:
  ...
  void MethodGenericInstance::addArguments(const type::Type *_node, TypeArgumentConstraintKind acValue) {
    if (_node == NULL)
    ...
    if (factory->reverseEdges)
      factory->reverseEdges->insertEdge(_node, this, edkMethodGenericInstance_HasArguments);
  }

  void MethodGenericInstance::addArguments(NodeId _id, TypeArgumentConstraintKind acValue) {
  ...
  void MethodGenericInstance::removeArguments(NodeId id) {
  ...
  void MethodGenericInstance::removeArguments(type::Type *_node) {
    if (_node == NULL)
  ...
  void MethodGenericInstance::accept(Visitor &visitor) const {
    visitor.visit(*this);
  }

  void MethodGenericInstance::acceptEnd(Visitor &visitor) const {
    visitor.visitEnd(*this);
  }

  double MethodGenericInstance::getSimilarity(const base::Base& base){
    if(base.getNodeKind() == getNodeKind()) {
      const MethodGenericInstance& node = dynamic_cast<const MethodGenericInstance&>(base);
      double matchAttrs = 0;
      ...
      return matchAttrs / (25 / (1 - Common::SimilarityMinimum)) + Common::SimilarityMinimum;
    } else {
      return 0.0;
    }
  }

  void MethodGenericInstance::swapStringTable(RefDistributorStrTable& newStrTable, std::map<Key,Key>& oldAndNewStrKeyMap ){
  ...
  NodeHashType MethodGenericInstance::getHash(std::set<NodeId>& travNodes) const {
    ...
    Crc_type resultHash;
    resultHash.process_bytes( "logical::MethodGenericInstance", strlen("logical::MethodGenericInstance"));
  ...
  void MethodGenericInstance::save(io::BinaryIO &binIo,bool withVirtualBase  /*= true*/) const {
  ...
  void MethodGenericInstance::load(io::BinaryIO &binIo, bool withVirtualBase /*= true*/) {
