...
#ifndef _LIM_MethodGenericInstance_H_
#define _LIM_MethodGenericInstance_H_

#include "lim/inc/lim.h"

/**
* \file MethodGenericInstance.h
* \brief Contains declaration of the logical::MethodGenericInstance class.
...
namespace logical {
  ...
  /**
  * \brief MethodGenericInstance class, which represents the logical::MethodGenericInstance node.
  * (missing)
  * 
  * Attributes:
  *   - isRealInstance (boolean) : (missing)
  * 
  * Edges:
  *   - hasArguments (type::Type, multiple) : (missing)
  */
  class MethodGenericInstance : public Method {
    protected:
      /**
      * \internal
      ...
      */
      MethodGenericInstance(NodeId nodeId, Factory *factory);

      /**
      * \internal
      * \brief Non-public destructor, only factory can destroy nodes.
      */
      virtual ~MethodGenericInstance();

    private:
      /**
      * \brief This function always throws a LimException due to copying is not allowed!
      */
      MethodGenericInstance & operator=(const MethodGenericInstance&);

      /**
      * \brief This function always throws a LimException due to copying is not allowed!
      */
      MethodGenericInstance(const MethodGenericInstance&);
    ...
    protected:
      ...
      * \brief Clone method.
      */
      virtual void clone(const MethodGenericInstance& other, bool tryOnVirtualParent);
    ...
    public:
      ...
      */
      ...
      /**
      * \brief Gives back the isRealInstance of the node.
      * \return Returns with the isRealInstance.
      */
      bool getIsRealInstance() const;
      ...
      /**
      * \internal
      * \brief Sets the isRealInstance of the node.
      * \param isRealInstance [in] The new value of the isRealInstance.
      */
      void setIsRealInstance(bool _isRealInstance);

    protected:

      // ---------- Attribute(s) ----------

      /** \internal \brief Stores whether the node is `RealInstance` or not. */
      bool m_isRealInstance : 1;
