...
namespace Columbus.Lim.Asg.Nodes.Logical {
    ...
    /// <summary>
    /// ClassGenericSpec class, which represents the logical::ClassGenericSpec node.
    /// </summary>
    ...
    public
#endif
    class ClassGenericSpec : ClassGeneric {
        ...
        public ClassGenericSpec(uint nodeId, Factory factory) : base(nodeId, factory) {
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkClassGenericSpec; }
        }
        ...
        public void addHasArguments(uint nodeId, Types.TypeArgumentConstraintKind acValue) {
            if (!fact.getExist(nodeId))
                throw new LimException("Columbus.Lim.Asg.Nodes.Logical.ClassGenericSpec.addHasArguments(NodeId)", "The end point of the edge does not exist.");
            ...
            if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkType)) {
                ...
                throw new LimException("Columbus.Lim.Asg.Nodes.Logical.ClassGenericSpec.setHasArguments(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
            }
            if (fact.ReverseEdges != null)
              fact.ReverseEdges.insertEdge(nodeId, this.Id, Types.EdgeKind.edkClassGenericSpec_HasArguments);
        }
        ...
        public void setSpecialize(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new LimException("Columbus.Lim.Asg.Nodes.Logical.ClassGenericSpec.setSpecialize(NodeId)", "The end point of the edge does not exist.");

                Columbus.Lim.Asg.Nodes.Base.Base node = fact.getRef(nodeId);
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkClassGeneric)) {
                    if (m_specialize != 0) {
                    }
                    m_specialize = nodeId;
                } else {
                    throw new LimException("Columbus.Lim.Asg.Nodes.Logical.ClassGenericSpec.setSpecialize(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_specialize != 0) {
                }
