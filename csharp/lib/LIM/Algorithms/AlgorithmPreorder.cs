  ...
  visitorList.Add( visitor);
        ...
        public void visitAllEdges(Columbus.Lim.Asg.Nodes.Base.Component node, bool callFirst = true){
            ...
            while (iteratorForcompilationUnit.getValue() != null) {
              ...
              foreach (var visitor in visitorList) {
                visitor.visitComponent_CompilationUnit(node.Id, endNodeRef.Id);
              }
              ...
              if ((!fact.IsFilterTurnedOn && !visitFilteredEdge) || (fact.getFilterState(endNodeRef.Id) == Filter.FilterState.NotFiltered) || (!originalFilterState && visitFilteredEdge)) {
                  ...
                  if (traversaldCrossEdges[(int)Types.EdgeKind.edkComponent_CompilationUnit]) {
                    endNodeRef.accept(this);
                  }
              }
              foreach (var visitor in visitorList) {
                visitor.visitEndComponent_CompilationUnit(node.Id, endNodeRef.Id);
              }
        ...
        public void visitAllEdges(Columbus.Lim.Asg.Nodes.Logical.Class node, bool callFirst = true){
            ...
            // edge: extends
            ListIterator<Columbus.Lim.Asg.Nodes.Logical.Class> iteratorForextends = node.ExtendsListIteratorBegin;
            while (iteratorForextends.getValue() != null) {
               Columbus.Lim.Asg.Nodes.Logical.Class endNodeRef = (Columbus.Lim.Asg.Nodes.Logical.Class)iteratorForextends.getValue();
              foreach (var visitor in visitorList) {
                visitor.visitClass_Extends(node.Id, endNodeRef.Id);
              }
              ...
              if ((!fact.IsFilterTurnedOn && !visitFilteredEdge) || (fact.getFilterState(endNodeRef.Id) == Filter.FilterState.NotFiltered) || (!originalFilterState && visitFilteredEdge)) {
                  ...
                  if (traversaldCrossEdges[(int)Types.EdgeKind.edkClass_Extends]) {
                    endNodeRef.accept(this);
                  }
              }
              foreach (var visitor in visitorList) {
                visitor.visitEndClass_Extends(node.Id, endNodeRef.Id);
              }
            ...
            iteratorForextends = iteratorForextends.getNext();
            }
