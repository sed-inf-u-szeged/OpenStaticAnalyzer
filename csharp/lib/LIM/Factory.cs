...
﻿        public Columbus.Lim.Asg.Nodes.Physical.File createFile(string path)
        ...
        public Nodes.Physical.File createFile(string path, int nodeId)
        {
            ...
            int found = 0;
            while ((pos = path.IndexOf(System.IO.Path.DirectorySeparatorChar, pos)) >= 0)
            {
                if (lastPos > 0)
                {
                ...
                {
                    name = path[0] != System.IO.Path.DirectorySeparatorChar ? path.Substring(0, pos) : System.IO.Path.DirectorySeparatorChar.ToString();
                }
