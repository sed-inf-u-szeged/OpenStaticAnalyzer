...
namespace Columbus.Lim.Asg.Nodes.Base {
    ...
    /// </summary>
    [DebuggerDisplay("Id={Id}, ShortName=\"{ShortName}\", Name=\"{Name}\"")]
#if (LIM_INTERNAL)
    internal
#else
    public
    ...
    class Component : Named {
        ...
        public void addCompilationUnit(uint nodeId) {
            if (!fact.getExist(nodeId))
                throw new LimException("Columbus.Lim.Asg.Nodes.Base.Component.addCompilationUnit(NodeId)", "The end point of the edge does not exist.");
            ...
            if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkFile)) {
                ...
                throw new LimException("Columbus.Lim.Asg.Nodes.Base.Component.setCompilationUnit(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
            }
            if (fact.ReverseEdges != null)
              fact.ReverseEdges.insertEdge(nodeId, this.Id, Types.EdgeKind.edkComponent_CompilationUnit);
        }
