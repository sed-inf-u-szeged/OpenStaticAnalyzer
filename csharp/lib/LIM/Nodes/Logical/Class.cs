...
namespace Columbus.Lim.Asg.Nodes.Logical {
    ...
    class Class : Scope {
        ...
        /// <summary>Container stores the id of the nodes the extends edge points to.</summary>
        protected LinkedList<uint> extendsContainer;
        ...
        public Class(uint nodeId, Factory factory) : base(nodeId, factory) {
            ...
            m_isAbstract = false;
            m_objectSize = 0;
            extendsContainer = new LinkedList<uint>();
        ...
        public override ListIterator<Columbus.Lim.Asg.Nodes.Base.Base> GetListIteratorBegin(string edge) {
            switch(edge) {
                case "Extends":
                    return ExtendsListIteratorBegin.Convert<Columbus.Lim.Asg.Nodes.Base.Base>();
        ...
        /// <summary>
        /// Gives back iterator for the extends edges.
        /// </summary>
        /// <returns>Returns an iterator for the extends edges.</returns>
        public ListIterator<Columbus.Lim.Asg.Nodes.Logical.Class> ExtendsListIteratorBegin {
            get {
                return new ListIterator<Columbus.Lim.Asg.Nodes.Logical.Class>(extendsContainer, fact, true);
            }
        }
        ...
        /// </summary>
        /// <returns>Returns an iterator for the extends edges.</returns>
        public ListIterator<Columbus.Lim.Asg.Nodes.Logical.Class> ExtendsListIteratorEnd {
            get {
                return new ListIterator<Columbus.Lim.Asg.Nodes.Logical.Class>(extendsContainer, fact, false);
            }
        }

        /// <summary>
        /// Tells whether the node has extends edges or not.
        /// </summary>
        /// <returns>Returns true if the node has at least one extends edge.</returns>
        public bool ExtendsIsEmpty {
            get { return extendsContainer.Count == 0; }
        }

        /// <summary>
        /// Gives back how many extends edges the node has.
        /// </summary>
        /// <returns>Returns with the number of extends edges.</returns>
        public uint ExtendsSize {
            get { return (uint)(extendsContainer.Count); }
        }
        ...
        /// <summary>
        /// Adds a new extends edge to the node and inserts it after the other ones.
        /// </summary>
        /// <param name="node">[in] The end point of the new extends edge.</param>
        ...
        public void addExtends(Columbus.Lim.Asg.Nodes.Logical.Class node) {
            addExtends(node.Id);
        }
        ...
        /// Adds a new extends edge to the node and inserts it after the other ones.
        /// </summary>
        /// <param name="nodeId">[in] The end point of the new extends edge.</param>
        /// <exception cref="Columbus.Lim.Asg.LimException">Throws LimException if the given nodeId is invalid.</exception>
        public void addExtends(uint nodeId) {
            if (!fact.getExist(nodeId))
                throw new LimException("Columbus.Lim.Asg.Nodes.Logical.Class.addExtends(NodeId)", "The end point of the edge does not exist.");
            Columbus.Lim.Asg.Nodes.Base.Base node = fact.getRef(nodeId);
            if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkClass)) {
                extendsContainer.AddLast(nodeId);
            } else {
                throw new LimException("Columbus.Lim.Asg.Nodes.Logical.Class.setExtends(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
            }
            if (fact.ReverseEdges != null)
              fact.ReverseEdges.insertEdge(nodeId, this.Id, Types.EdgeKind.edkClass_Extends);
        }
        ...
        public override void save(IO io) {
            ...
            foreach (var extends in extendsContainer) {
              io.writeUInt4(extends);
            }
