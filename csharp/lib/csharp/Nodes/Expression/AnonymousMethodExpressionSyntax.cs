...
namespace Columbus.Csharp.Asg.Nodes.Expression {
    ...
    /// <summary>
    /// AnonymousMethodExpressionSyntax class, which represents the expression::AnonymousMethodExpressionSyntax node.
    /// </summary>
    ...
    class AnonymousMethodExpressionSyntax : AnonymousFunctionExpressionSyntax {
        ...
        public AnonymousMethodExpressionSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
            m_ParameterList = 0;
        }
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkAnonymousMethodExpressionSyntax; }
        }
        ...
        public void setParameterList(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.AnonymousMethodExpressionSyntax.setParameterList(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkParameterListSyntax)) {
                    ...
                    setParentEdge(m_ParameterList, (uint)Types.EdgeKind.edkAnonymousMethodExpressionSyntax_ParameterList);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.AnonymousMethodExpressionSyntax.setParameterList(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_ParameterList != 0) {
                    removeParentEdge(m_ParameterList);
