...
namespace Columbus.Csharp.Asg.Nodes.Expression {
    ...
    /// <summary>
    /// PostfixUnaryExpressionSyntax class, which represents the expression::PostfixUnaryExpressionSyntax node.
    /// </summary>
    ...
    class PostfixUnaryExpressionSyntax : ExpressionSyntax {
        ...
        public PostfixUnaryExpressionSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
            m_Operand = 0;
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkPostfixUnaryExpressionSyntax; }
        }
        ...
        public void setOverloadedOperatorCall(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.PostfixUnaryExpressionSyntax.setOverloadedOperatorCall(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkOperatorDeclarationSyntax)) {
                    ...
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.PostfixUnaryExpressionSyntax.setOverloadedOperatorCall(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_OverloadedOperatorCall != 0) {
                }
