...
namespace Columbus.Csharp.Asg.Nodes.Expression {
    ...
    /// <summary>
    /// NullableTypeSyntax class, which represents the expression::NullableTypeSyntax node.
    /// </summary>
    ...
    class NullableTypeSyntax : TypeSyntax {
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkNullableTypeSyntax; }
        }
        ...
        public void setElementType(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.NullableTypeSyntax.setElementType(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkTypeSyntax)) {
                    ...
                    setParentEdge(m_ElementType, (uint)Types.EdgeKind.edkNullableTypeSyntax_ElementType);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.NullableTypeSyntax.setElementType(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_ElementType != 0) {
                    removeParentEdge(m_ElementType);
