...
namespace Columbus.Csharp.Asg.Nodes.Expression {
    ...
    /// <summary>
    /// ImplicitElementAccessSyntax class, which represents the expression::ImplicitElementAccessSyntax node.
    /// </summary>
    ...
    class ImplicitElementAccessSyntax : ExpressionSyntax {
        ...
        public ImplicitElementAccessSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
            m_ArgumentList = 0;
        }
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkImplicitElementAccessSyntax; }
        }
        ...
        public void setArgumentList(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.ImplicitElementAccessSyntax.setArgumentList(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkBracketedArgumentListSyntax)) {
                    ...
                    setParentEdge(m_ArgumentList, (uint)Types.EdgeKind.edkImplicitElementAccessSyntax_ArgumentList);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.ImplicitElementAccessSyntax.setArgumentList(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_ArgumentList != 0) {
                    removeParentEdge(m_ArgumentList);
