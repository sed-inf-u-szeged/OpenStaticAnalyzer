...
namespace Columbus.Csharp.Asg.Nodes.Structure {
    ...
    /// <summary>
    /// ConversionOperatorDeclarationSyntax class, which represents the structure::ConversionOperatorDeclarationSyntax node.
    /// </summary>
    ...
    class ConversionOperatorDeclarationSyntax : BaseMethodDeclarationSyntax {
        ...
        public ConversionOperatorDeclarationSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
            m_ExpressionBody = 0;
            m_Type = 0;
        }
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkConversionOperatorDeclarationSyntax; }
        }
        ...
        public void setExpressionBody(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.ConversionOperatorDeclarationSyntax.setExpressionBody(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkArrowExpressionClauseSyntax)) {
                    ...
                    setParentEdge(m_ExpressionBody, (uint)Types.EdgeKind.edkConversionOperatorDeclarationSyntax_ExpressionBody);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.ConversionOperatorDeclarationSyntax.setExpressionBody(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_ExpressionBody != 0) {
                    removeParentEdge(m_ExpressionBody);
