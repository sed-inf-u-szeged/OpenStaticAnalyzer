...
namespace Columbus.Csharp.Asg.Nodes.Structure {
    ...
    /// <summary>
    /// ElseClauseSyntax class, which represents the structure::ElseClauseSyntax node.
    /// </summary>
    ...
    public
#endif
    class ElseClauseSyntax : Columbus.Csharp.Asg.Nodes.Base.Positioned {
        ...
        public ElseClauseSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
            m_Statement = 0;
        }
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkElseClauseSyntax; }
        }
        ...
        public void setStatement(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.ElseClauseSyntax.setStatement(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkStatementSyntax)) {
                    ...
                    m_Statement = nodeId;
                    setParentEdge(m_Statement, (uint)Types.EdgeKind.edkElseClauseSyntax_Statement);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.ElseClauseSyntax.setStatement(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Statement != 0) {
                    removeParentEdge(m_Statement);
