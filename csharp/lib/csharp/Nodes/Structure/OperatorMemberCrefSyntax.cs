...
namespace Columbus.Csharp.Asg.Nodes.Structure {
    ...
    /// <summary>
    /// OperatorMemberCrefSyntax class, which represents the structure::OperatorMemberCrefSyntax node.
    /// </summary>
    ...
    class OperatorMemberCrefSyntax : MemberCrefSyntax {
        ...
        public OperatorMemberCrefSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
            m_Parameters = 0;
        }
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkOperatorMemberCrefSyntax; }
        }
        ...
        public void setParameters(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.OperatorMemberCrefSyntax.setParameters(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkCrefParameterListSyntax)) {
                    ...
                    m_Parameters = nodeId;
                    setParentEdge(m_Parameters, (uint)Types.EdgeKind.edkOperatorMemberCrefSyntax_Parameters);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.OperatorMemberCrefSyntax.setParameters(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Parameters != 0) {
                    removeParentEdge(m_Parameters);
