...
namespace Columbus.Csharp.Asg.Nodes.Structure {
    ...
    /// <summary>
    /// QueryContinuationSyntax class, which represents the structure::QueryContinuationSyntax node.
    /// </summary>
    ...
    public
#endif
    class QueryContinuationSyntax : Columbus.Csharp.Asg.Nodes.Base.Positioned {
        ...
        public QueryContinuationSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
            m_identifier = 0;
            m_Body = 0;
        }
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkQueryContinuationSyntax; }
        }
        ...
        public void setBody(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.QueryContinuationSyntax.setBody(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkQueryBodySyntax)) {
                    ...
                    m_Body = nodeId;
                    setParentEdge(m_Body, (uint)Types.EdgeKind.edkQueryContinuationSyntax_Body);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.QueryContinuationSyntax.setBody(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Body != 0) {
                    removeParentEdge(m_Body);
