...
namespace Columbus.Csharp.Asg.Nodes.Structure {
    ...
    /// <summary>
    /// UsingDirectiveSyntax class, which represents the structure::UsingDirectiveSyntax node.
    /// </summary>
    ...
    public
#endif
    class UsingDirectiveSyntax : Columbus.Csharp.Asg.Nodes.Base.Positioned {
        ...
        public UsingDirectiveSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
            m_Alias = 0;
            m_Name = 0;
        }
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkUsingDirectiveSyntax; }
        }
        ...
        public void setName(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.UsingDirectiveSyntax.setName(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkNameSyntax)) {
                    ...
                    m_Name = nodeId;
                    setParentEdge(m_Name, (uint)Types.EdgeKind.edkUsingDirectiveSyntax_Name);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.UsingDirectiveSyntax.setName(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Name != 0) {
                    removeParentEdge(m_Name);
