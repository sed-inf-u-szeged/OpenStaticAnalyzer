...
namespace Columbus.Csharp.Asg.Nodes.Structure {
    ...
    /// <summary>
    /// IndexerDeclarationSyntax class, which represents the structure::IndexerDeclarationSyntax node.
    /// </summary>
    ...
    class IndexerDeclarationSyntax : BasePropertyDeclarationSyntax {
        ...
        public IndexerDeclarationSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
            m_ExpressionBody = 0;
            m_ParameterList = 0;
        }
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkIndexerDeclarationSyntax; }
        }
        ...
        public void setParameterList(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.IndexerDeclarationSyntax.setParameterList(NodeId)", "The end point of the edge does not exist.");

                Columbus.Csharp.Asg.Nodes.Base.Base node = fact.getRef(nodeId);
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkBracketedParameterListSyntax)) {
                    if (m_ParameterList != 0) {
                        removeParentEdge(m_ParameterList);
                    }
                    m_ParameterList = nodeId;
                    setParentEdge(m_ParameterList, (uint)Types.EdgeKind.edkIndexerDeclarationSyntax_ParameterList);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.IndexerDeclarationSyntax.setParameterList(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_ParameterList != 0) {
                    removeParentEdge(m_ParameterList);
        ...
        /// <exception cref="Columbus.Csharp.Asg.CsharpException">Throws CsharpException if there's something wrong with the given node.</exception>
        public void setParameterList(Columbus.Csharp.Asg.Nodes.Structure.BracketedParameterListSyntax node) {
            if (m_ParameterList != 0) {
                removeParentEdge(m_ParameterList);
            }
