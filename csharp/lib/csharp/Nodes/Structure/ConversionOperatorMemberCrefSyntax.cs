...
namespace Columbus.Csharp.Asg.Nodes.Structure {
    ...
    /// <summary>
    /// ConversionOperatorMemberCrefSyntax class, which represents the structure::ConversionOperatorMemberCrefSyntax node.
    /// </summary>
    ...
    class ConversionOperatorMemberCrefSyntax : MemberCrefSyntax {
        ...
        public ConversionOperatorMemberCrefSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
            m_Parameters = 0;
            m_Type = 0;
        }
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkConversionOperatorMemberCrefSyntax; }
        }
        ...
        public void setParameters(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.ConversionOperatorMemberCrefSyntax.setParameters(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkCrefParameterListSyntax)) {
                    ...
                    m_Parameters = nodeId;
                    setParentEdge(m_Parameters, (uint)Types.EdgeKind.edkConversionOperatorMemberCrefSyntax_Parameters);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.ConversionOperatorMemberCrefSyntax.setParameters(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Parameters != 0) {
                    removeParentEdge(m_Parameters);
        ...
        public void setType(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.ConversionOperatorMemberCrefSyntax.setType(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkTypeSyntax)) {
                    ...
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.ConversionOperatorMemberCrefSyntax.setType(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Type != 0) {
                    removeParentEdge(m_Type);
