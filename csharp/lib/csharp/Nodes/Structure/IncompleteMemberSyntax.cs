...
namespace Columbus.Csharp.Asg.Nodes.Structure {
    ...
    /// <summary>
    /// IncompleteMemberSyntax class, which represents the structure::IncompleteMemberSyntax node.
    /// </summary>
    ...
    class IncompleteMemberSyntax : MemberDeclarationSyntax {
        ...
        public IncompleteMemberSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkIncompleteMemberSyntax; }
        }
        ...
        public void addAttributeLists(uint nodeId) {
            if (!fact.getExist(nodeId))
                throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.IncompleteMemberSyntax.addAttributeLists(NodeId)", "The end point of the edge does not exist.");
            ...
            if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkAttributeListSyntax)) {
                ...
                throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.IncompleteMemberSyntax.setAttributeLists(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
            }
           setParentEdge(nodeId, (uint)Types.EdgeKind.edkIncompleteMemberSyntax_AttributeLists);
            if (fact.ReverseEdges != null)
              fact.ReverseEdges.insertEdge(nodeId, this.Id, Types.EdgeKind.edkIncompleteMemberSyntax_AttributeLists);
        }
        ...
        public void setType(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.IncompleteMemberSyntax.setType(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkTypeSyntax)) {
                    ...
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.IncompleteMemberSyntax.setType(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Type != 0) {
                    removeParentEdge(m_Type);
        ...
        public override void load(IO binIo) {
            ...
            while (_id != 0) {
              ...
              setParentEdge(_id, (uint)Types.EdgeKind.edkIncompleteMemberSyntax_AttributeLists);
              _id = binIo.readUInt4();
            }
        }
