...
namespace Columbus.Csharp.Asg.Nodes.Structure {
    ...
    /// <summary>
    /// XmlProcessingInstructionSyntax class, which represents the structure::XmlProcessingInstructionSyntax node.
    /// </summary>
    ...
    class XmlProcessingInstructionSyntax : XmlNodeSyntax {
        ...
        public XmlProcessingInstructionSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
            m_Name = 0;
        }
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkXmlProcessingInstructionSyntax; }
        }
        ...
        public void setName(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.XmlProcessingInstructionSyntax.setName(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkXmlNameSyntax)) {
                    ...
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.XmlProcessingInstructionSyntax.setName(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Name != 0) {
                    removeParentEdge(m_Name);
