...
namespace Columbus.Csharp.Asg.Nodes.Structure {
    ...
    /// <summary>
    /// NameEqualsSyntax class, which represents the structure::NameEqualsSyntax node.
    /// </summary>
    ...
    public
#endif
    class NameEqualsSyntax : Columbus.Csharp.Asg.Nodes.Base.Positioned {
        ...
        public NameEqualsSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
            m_Name = 0;
        }
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkNameEqualsSyntax; }
        }
        ...
        public void setName(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.NameEqualsSyntax.setName(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkIdentifierNameSyntax)) {
                    ...
                    m_Name = nodeId;
                    setParentEdge(m_Name, (uint)Types.EdgeKind.edkNameEqualsSyntax_Name);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.NameEqualsSyntax.setName(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Name != 0) {
                    removeParentEdge(m_Name);
