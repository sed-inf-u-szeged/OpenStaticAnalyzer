...
namespace Columbus.Csharp.Asg.Nodes.Structure {
    ...
    /// <summary>
    /// CaseSwitchLabelSyntax class, which represents the structure::CaseSwitchLabelSyntax node.
    /// </summary>
    ...
    class CaseSwitchLabelSyntax : SwitchLabelSyntax {
        ...
        public CaseSwitchLabelSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
            m_Value = 0;
        }
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkCaseSwitchLabelSyntax; }
        }
        ...
        public void setValue(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.CaseSwitchLabelSyntax.setValue(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkExpressionSyntax)) {
                    ...
                    m_Value = nodeId;
                    setParentEdge(m_Value, (uint)Types.EdgeKind.edkCaseSwitchLabelSyntax_Value);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.CaseSwitchLabelSyntax.setValue(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Value != 0) {
                    removeParentEdge(m_Value);
