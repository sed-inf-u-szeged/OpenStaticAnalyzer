...
namespace Columbus.Csharp.Asg.Nodes.Statement {
    ...
    /// <summary>
    /// IfStatementSyntax class, which represents the statement::IfStatementSyntax node.
    /// </summary>
    ...
    class IfStatementSyntax : StatementSyntax {
        ...
        /// <summary>The id of the node the Else edge points to.</summary>
        protected uint m_Else;
        ...
        public IfStatementSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
            m_Condition = 0;
            m_Else = 0;
            m_Statement = 0;
        }
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkIfStatementSyntax; }
        }
        ...
        /// <summary>
        /// Gives back the id of the node the Else edge points to.
        /// </summary>
        /// <returns>Returns the end point of the m_Else edge.</returns>
        /// <exception cref="Columbus.Csharp.Asg.CsharpException">Throws CsharpException if the edge id is invalid.</exception>
        public uint getElse() {
            if (fact.getIsFiltered(m_Else))
                return 1;
            else
                return m_Else;
        }
        ...
        public void setCondition(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Statement.IfStatementSyntax.setCondition(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkExpressionSyntax)) {
                    ...
                    m_Condition = nodeId;
                    setParentEdge(m_Condition, (uint)Types.EdgeKind.edkIfStatementSyntax_Condition);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Statement.IfStatementSyntax.setCondition(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Condition != 0) {
                    removeParentEdge(m_Condition);
        ...
        /// Sets the Else edge.
        /// </summary>
        /// <param name="node">[in] The new end point of the Else edge.</param>
        /// <exception cref="Columbus.Csharp.Asg.CsharpException">Throws CsharpException if there's something wrong with the given node.</exception>
        public void setElse(Columbus.Csharp.Asg.Nodes.Structure.ElseClauseSyntax node) {
            if (m_Else != 0) {
                removeParentEdge(m_Else);
            }
            m_Else = node.Id;
        ...
        public void setStatement(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Statement.IfStatementSyntax.setStatement(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkStatementSyntax)) {
                    ...
                    m_Statement = nodeId;
                    setParentEdge(m_Statement, (uint)Types.EdgeKind.edkIfStatementSyntax_Statement);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Statement.IfStatementSyntax.setStatement(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Statement != 0) {
                    removeParentEdge(m_Statement);
