...
namespace Columbus.Csharp.Asg.Nodes.Statement {
    ...
    /// <summary>
    /// CheckedStatementSyntax class, which represents the statement::CheckedStatementSyntax node.
    /// </summary>
    ...
    class CheckedStatementSyntax : StatementSyntax {
        ...
        public CheckedStatementSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
            m_Block = 0;
        }
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkCheckedStatementSyntax; }
        }
        ...
        public void setBlock(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Statement.CheckedStatementSyntax.setBlock(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkBlockSyntax)) {
                    ...
                    m_Block = nodeId;
                    setParentEdge(m_Block, (uint)Types.EdgeKind.edkCheckedStatementSyntax_Block);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Statement.CheckedStatementSyntax.setBlock(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Block != 0) {
                    removeParentEdge(m_Block);
