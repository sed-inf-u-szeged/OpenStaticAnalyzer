...
namespace Columbus.Csharp.Asg.Nodes.Statement {
    ...
    /// <summary>
    /// WhileStatementSyntax class, which represents the statement::WhileStatementSyntax node.
    /// </summary>
    ...
    class WhileStatementSyntax : StatementSyntax {
        ...
        public WhileStatementSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
            m_Condition = 0;
            m_Statement = 0;
        }
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkWhileStatementSyntax; }
        }
        ...
        public void setCondition(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Statement.WhileStatementSyntax.setCondition(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkExpressionSyntax)) {
                    ...
                    m_Condition = nodeId;
                    setParentEdge(m_Condition, (uint)Types.EdgeKind.edkWhileStatementSyntax_Condition);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Statement.WhileStatementSyntax.setCondition(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Condition != 0) {
                    removeParentEdge(m_Condition);
        ...
        public void setStatement(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Statement.WhileStatementSyntax.setStatement(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkStatementSyntax)) {
                    ...
                    m_Statement = nodeId;
                    setParentEdge(m_Statement, (uint)Types.EdgeKind.edkWhileStatementSyntax_Statement);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Statement.WhileStatementSyntax.setStatement(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Statement != 0) {
                    removeParentEdge(m_Statement);
