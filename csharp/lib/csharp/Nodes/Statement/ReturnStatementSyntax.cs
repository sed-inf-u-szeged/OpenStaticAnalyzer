...
namespace Columbus.Csharp.Asg.Nodes.Statement {
    ...
    /// <summary>
    /// ReturnStatementSyntax class, which represents the statement::ReturnStatementSyntax node.
    /// </summary>
    ...
    class ReturnStatementSyntax : StatementSyntax {
        ...
        public ReturnStatementSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
            m_Expression = 0;
        }
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkReturnStatementSyntax; }
        }
        ...
        public void setExpression(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Statement.ReturnStatementSyntax.setExpression(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkExpressionSyntax)) {
                    ...
                    m_Expression = nodeId;
                    setParentEdge(m_Expression, (uint)Types.EdgeKind.edkReturnStatementSyntax_Expression);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Statement.ReturnStatementSyntax.setExpression(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Expression != 0) {
                    removeParentEdge(m_Expression);
