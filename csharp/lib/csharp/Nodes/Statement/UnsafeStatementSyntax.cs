...
namespace Columbus.Csharp.Asg.Nodes.Statement {
    ...
    /// <summary>
    /// UnsafeStatementSyntax class, which represents the statement::UnsafeStatementSyntax node.
    /// </summary>
    ...
    class UnsafeStatementSyntax : StatementSyntax {
        ...
        public UnsafeStatementSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
            m_Block = 0;
        }
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkUnsafeStatementSyntax; }
        }
        ...
        public void setBlock(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Statement.UnsafeStatementSyntax.setBlock(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkBlockSyntax)) {
                    ...
                    m_Block = nodeId;
                    setParentEdge(m_Block, (uint)Types.EdgeKind.edkUnsafeStatementSyntax_Block);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Statement.UnsafeStatementSyntax.setBlock(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Block != 0) {
                    removeParentEdge(m_Block);
