...
namespace Columbus.Csharp.Asg.Nodes.Structure {
    ...
    /// <summary>
    /// AttributeSyntax class, which represents the structure::AttributeSyntax node.
    /// </summary>
    ...
    public
#endif
    class AttributeSyntax : Columbus.Csharp.Asg.Nodes.Base.Positioned {
        ...
        public AttributeSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
            m_ArgumentList = 0;
            m_Name = 0;
        }
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkAttributeSyntax; }
        }
        ...
        public void setArgumentList(Columbus.Csharp.Asg.Nodes.Structure.AttributeArgumentListSyntax node) {
            if (m_ArgumentList != 0) {
                removeParentEdge(m_ArgumentList);
            }
        ...
        public void setName(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.AttributeSyntax.setName(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkNameSyntax)) {
                    ...
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.AttributeSyntax.setName(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Name != 0) {
                    removeParentEdge(m_Name);
