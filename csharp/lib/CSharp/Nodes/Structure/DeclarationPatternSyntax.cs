...
namespace Columbus.Csharp.Asg.Nodes.Structure {
    ...
    /// <summary>
    /// DeclarationPatternSyntax class, which represents the structure::DeclarationPatternSyntax node.
    /// </summary>
    ...
    class DeclarationPatternSyntax : PatternSyntax {
        ...
        public DeclarationPatternSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
            m_Designation = 0;
            m_Type = 0;
        }
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkDeclarationPatternSyntax; }
        }
        ...
        public void setDesignation(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.DeclarationPatternSyntax.setDesignation(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkVariableDesignationSyntax)) {
                    ...
                    setParentEdge(m_Designation, (uint)Types.EdgeKind.edkDeclarationPatternSyntax_Designation);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.DeclarationPatternSyntax.setDesignation(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Designation != 0) {
                    removeParentEdge(m_Designation);
        ...
        public void setType(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.DeclarationPatternSyntax.setType(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkTypeSyntax)) {
                    ...
                    m_Type = nodeId;
                    setParentEdge(m_Type, (uint)Types.EdgeKind.edkDeclarationPatternSyntax_Type);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.DeclarationPatternSyntax.setType(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Type != 0) {
                    removeParentEdge(m_Type);
