...
namespace Columbus.Csharp.Asg.Nodes.Structure {
    ...
    /// <summary>
    /// WhenClauseSyntax class, which represents the structure::WhenClauseSyntax node.
    /// </summary>
    ...
    public
#endif
    class WhenClauseSyntax : Columbus.Csharp.Asg.Nodes.Base.Positioned {
        ...
        public WhenClauseSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
            m_Condition = 0;
        }
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkWhenClauseSyntax; }
        }
        ...
        public void setCondition(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.WhenClauseSyntax.setCondition(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkExpressionSyntax)) {
                    ...
                    m_Condition = nodeId;
                    setParentEdge(m_Condition, (uint)Types.EdgeKind.edkWhenClauseSyntax_Condition);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.WhenClauseSyntax.setCondition(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Condition != 0) {
                    removeParentEdge(m_Condition);
