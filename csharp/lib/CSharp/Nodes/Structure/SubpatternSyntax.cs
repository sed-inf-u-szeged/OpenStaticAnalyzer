...
namespace Columbus.Csharp.Asg.Nodes.Structure {
    ...
    /// <summary>
    /// SubpatternSyntax class, which represents the structure::SubpatternSyntax node.
    /// </summary>
    ...
    public
#endif
    class SubpatternSyntax : Columbus.Csharp.Asg.Nodes.Base.Positioned {
        ...
        public SubpatternSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
            m_Pattern = 0;
        }
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkSubpatternSyntax; }
        }
        ...
        public void setPattern(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.SubpatternSyntax.setPattern(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkPatternSyntax)) {
                    ...
                    m_Pattern = nodeId;
                    setParentEdge(m_Pattern, (uint)Types.EdgeKind.edkSubpatternSyntax_Pattern);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.SubpatternSyntax.setPattern(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Pattern != 0) {
                    removeParentEdge(m_Pattern);
