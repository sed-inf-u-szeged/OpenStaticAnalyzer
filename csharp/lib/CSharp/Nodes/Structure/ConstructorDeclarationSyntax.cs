...
namespace Columbus.Csharp.Asg.Nodes.Structure {
    ...
    /// <summary>
    /// ConstructorDeclarationSyntax class, which represents the structure::ConstructorDeclarationSyntax node.
    /// </summary>
    ...
    class ConstructorDeclarationSyntax : BaseMethodDeclarationSyntax {
        ...
        public ConstructorDeclarationSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
            m_identifier = 0;
            m_Initializer = 0;
        }
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkConstructorDeclarationSyntax; }
        }
        ...
        public void setInitializer(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.ConstructorDeclarationSyntax.setInitializer(NodeId)", "The end point of the edge does not exist.");

                Columbus.Csharp.Asg.Nodes.Base.Base node = fact.getRef(nodeId);
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkConstructorInitializerSyntax)) {
                    if (m_Initializer != 0) {
                        removeParentEdge(m_Initializer);
                    }
                    m_Initializer = nodeId;
                    setParentEdge(m_Initializer, (uint)Types.EdgeKind.edkConstructorDeclarationSyntax_Initializer);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.ConstructorDeclarationSyntax.setInitializer(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Initializer != 0) {
                    removeParentEdge(m_Initializer);
        ...
        public void setInitializer(Columbus.Csharp.Asg.Nodes.Structure.ConstructorInitializerSyntax node) {
            if (m_Initializer != 0) {
                removeParentEdge(m_Initializer);
            }
