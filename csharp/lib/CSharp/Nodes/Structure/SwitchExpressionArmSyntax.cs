...
namespace Columbus.Csharp.Asg.Nodes.Structure {
    ...
    /// <summary>
    /// SwitchExpressionArmSyntax class, which represents the structure::SwitchExpressionArmSyntax node.
    /// </summary>
    ...
    public
#endif
    class SwitchExpressionArmSyntax : Columbus.Csharp.Asg.Nodes.Base.Positioned {
        ...
        public SwitchExpressionArmSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
            m_Expression = 0;
            m_Pattern = 0;
            m_WhenClause = 0;
        }
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkSwitchExpressionArmSyntax; }
        }
        ...
        public void setExpression(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.SwitchExpressionArmSyntax.setExpression(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkExpressionSyntax)) {
                    ...
                    m_Expression = nodeId;
                    setParentEdge(m_Expression, (uint)Types.EdgeKind.edkSwitchExpressionArmSyntax_Expression);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.SwitchExpressionArmSyntax.setExpression(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Expression != 0) {
                    removeParentEdge(m_Expression);
        ...
        public void setPattern(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.SwitchExpressionArmSyntax.setPattern(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkPatternSyntax)) {
                    ...
                    m_Pattern = nodeId;
                    setParentEdge(m_Pattern, (uint)Types.EdgeKind.edkSwitchExpressionArmSyntax_Pattern);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.SwitchExpressionArmSyntax.setPattern(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Pattern != 0) {
                    removeParentEdge(m_Pattern);
        ...
        public void setWhenClause(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.SwitchExpressionArmSyntax.setWhenClause(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkWhenClauseSyntax)) {
                    ...
                    m_WhenClause = nodeId;
                    setParentEdge(m_WhenClause, (uint)Types.EdgeKind.edkSwitchExpressionArmSyntax_WhenClause);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.SwitchExpressionArmSyntax.setWhenClause(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_WhenClause != 0) {
                    removeParentEdge(m_WhenClause);
