...
namespace Columbus.Csharp.Asg.Nodes.Structure {
    ...
    /// <summary>
    /// XmlAttributeSyntax class, which represents the structure::XmlAttributeSyntax node.
    /// </summary>
    ...
    public
#endif
    class XmlAttributeSyntax : Columbus.Csharp.Asg.Nodes.Base.Positioned {
        ...
        public XmlAttributeSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
            m_Name = 0;
        }
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkXmlAttributeSyntax; }
        }
        ...
        public void setName(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.XmlAttributeSyntax.setName(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkXmlNameSyntax)) {
                    ...
                    m_Name = nodeId;
                    setParentEdge(m_Name, (uint)Types.EdgeKind.edkXmlAttributeSyntax_Name);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.XmlAttributeSyntax.setName(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Name != 0) {
                    removeParentEdge(m_Name);
