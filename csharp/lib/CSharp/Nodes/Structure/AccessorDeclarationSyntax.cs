...
namespace Columbus.Csharp.Asg.Nodes.Structure {
    ...
    class AccessorDeclarationSyntax : Columbus.Csharp.Asg.Nodes.Base.Positioned {
        ...
        public void setExpressionBody(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.AccessorDeclarationSyntax.setExpressionBody(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkArrowExpressionClauseSyntax)) {
                    ...
                    setParentEdge(m_ExpressionBody, (uint)Types.EdgeKind.edkAccessorDeclarationSyntax_ExpressionBody);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.AccessorDeclarationSyntax.setExpressionBody(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_ExpressionBody != 0) {
                    removeParentEdge(m_ExpressionBody);
