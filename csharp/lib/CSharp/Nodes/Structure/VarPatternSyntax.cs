...
namespace Columbus.Csharp.Asg.Nodes.Structure {
    ...
    /// <summary>
    /// VarPatternSyntax class, which represents the structure::VarPatternSyntax node.
    /// </summary>
    ...
    class VarPatternSyntax : PatternSyntax {
        ...
        public VarPatternSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
            m_Designation = 0;
        }
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkVarPatternSyntax; }
        }
        ...
        public void setDesignation(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.VarPatternSyntax.setDesignation(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkVariableDesignationSyntax)) {
                    ...
                    setParentEdge(m_Designation, (uint)Types.EdgeKind.edkVarPatternSyntax_Designation);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.VarPatternSyntax.setDesignation(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Designation != 0) {
                    removeParentEdge(m_Designation);
