...
namespace Columbus.Csharp.Asg.Nodes.Structure {
    ...
    /// <summary>
    /// EqualsValueClauseSyntax class, which represents the structure::EqualsValueClauseSyntax node.
    /// </summary>
    ...
    public
#endif
    class EqualsValueClauseSyntax : Columbus.Csharp.Asg.Nodes.Base.Positioned {
        ...
        public EqualsValueClauseSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
            m_Value = 0;
        }
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkEqualsValueClauseSyntax; }
        }
        ...
        public void setValue(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.EqualsValueClauseSyntax.setValue(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkExpressionSyntax)) {
                    ...
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Structure.EqualsValueClauseSyntax.setValue(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Value != 0) {
                    removeParentEdge(m_Value);
