...
namespace Columbus.Csharp.Asg.Nodes.Expression {
    ...
    /// <summary>
    /// AssignmentExpressionSyntax class, which represents the expression::AssignmentExpressionSyntax node.
    /// </summary>
    ...
    class AssignmentExpressionSyntax : ExpressionSyntax {
        ...
        /// <summary>The id of the node the EventAccessorCall edge points to.</summary>
        protected uint m_EventAccessorCall;
        ...
        public AssignmentExpressionSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkAssignmentExpressionSyntax; }
        }
        ...
        /// <summary>
        /// Gives back the id of the node the EventAccessorCall edge points to.
        /// </summary>
        /// <returns>Returns the end point of the m_EventAccessorCall edge.</returns>
        /// <exception cref="Columbus.Csharp.Asg.CsharpException">Throws CsharpException if the edge id is invalid.</exception>
        public uint getEventAccessorCall() {
            if (fact.getIsFiltered(m_EventAccessorCall))
                return 1;
            else
                return m_EventAccessorCall;
        }
        ...
        /// </summary>
        /// <param name="nodeId">[in] The new end point of the EventAccessorCall edge or 0. With 0 the edge can be deleted.</param>
        /// <exception cref="Columbus.Csharp.Asg.CsharpException">Throws CsharpException if the nodeId is invalid.</exception>
        public void setEventAccessorCall(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.AssignmentExpressionSyntax.setEventAccessorCall(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkAccessorDeclarationSyntax)) {
                    ...
                    m_EventAccessorCall = nodeId;
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.AssignmentExpressionSyntax.setEventAccessorCall(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_EventAccessorCall != 0) {
                }
        ...
        public void setLeft(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.AssignmentExpressionSyntax.setLeft(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkExpressionSyntax)) {
                    ...
                    m_Left = nodeId;
                    setParentEdge(m_Left, (uint)Types.EdgeKind.edkAssignmentExpressionSyntax_Left);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.AssignmentExpressionSyntax.setLeft(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Left != 0) {
                    removeParentEdge(m_Left);
        ...
        public void setOverloadedOperatorCall(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.AssignmentExpressionSyntax.setOverloadedOperatorCall(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkOperatorDeclarationSyntax)) {
                    ...
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.AssignmentExpressionSyntax.setOverloadedOperatorCall(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_OverloadedOperatorCall != 0) {
                }
        ...
        public void setRight(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.AssignmentExpressionSyntax.setRight(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkExpressionSyntax)) {
                    ...
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.AssignmentExpressionSyntax.setRight(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Right != 0) {
                    removeParentEdge(m_Right);
        ...
        public override void save(IO io) {
            ...
            io.writeUInt4(m_EventAccessorCall);
