...
namespace Columbus.Csharp.Asg.Nodes.Expression {
    ...
    /// <summary>
    /// StackAllocArrayCreationExpressionSyntax class, which represents the expression::StackAllocArrayCreationExpressionSyntax node.
    /// </summary>
    ...
    class StackAllocArrayCreationExpressionSyntax : ExpressionSyntax {
        ...
        public StackAllocArrayCreationExpressionSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
            m_Initializer = 0;
            m_Type = 0;
        }
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkStackAllocArrayCreationExpressionSyntax; }
        }
        ...
        public void setInitializer(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.StackAllocArrayCreationExpressionSyntax.setInitializer(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkInitializerExpressionSyntax)) {
                    ...
                    setParentEdge(m_Initializer, (uint)Types.EdgeKind.edkStackAllocArrayCreationExpressionSyntax_Initializer);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.StackAllocArrayCreationExpressionSyntax.setInitializer(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Initializer != 0) {
                    removeParentEdge(m_Initializer);
        ...
        public void setType(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.StackAllocArrayCreationExpressionSyntax.setType(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkTypeSyntax)) {
                    ...
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.StackAllocArrayCreationExpressionSyntax.setType(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Type != 0) {
                    removeParentEdge(m_Type);
