...
namespace Columbus.Csharp.Asg.Nodes.Expression {
    ...
    /// <summary>
    /// ImplicitStackAllocArrayCreationExpressionSyntax class, which represents the expression::ImplicitStackAllocArrayCreationExpressionSyntax node.
    /// </summary>
    ...
    class ImplicitStackAllocArrayCreationExpressionSyntax : ExpressionSyntax {
        ...
        public ImplicitStackAllocArrayCreationExpressionSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
            m_Initializer = 0;
        }
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkImplicitStackAllocArrayCreationExpressionSyntax; }
        }
        ...
        public void setInitializer(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.ImplicitStackAllocArrayCreationExpressionSyntax.setInitializer(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkInitializerExpressionSyntax)) {
                    ...
                    setParentEdge(m_Initializer, (uint)Types.EdgeKind.edkImplicitStackAllocArrayCreationExpressionSyntax_Initializer);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.ImplicitStackAllocArrayCreationExpressionSyntax.setInitializer(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Initializer != 0) {
                    removeParentEdge(m_Initializer);
