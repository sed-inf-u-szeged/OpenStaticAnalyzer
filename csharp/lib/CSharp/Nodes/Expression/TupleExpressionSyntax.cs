...
namespace Columbus.Csharp.Asg.Nodes.Expression {
    ...
    /// <summary>
    /// TupleExpressionSyntax class, which represents the expression::TupleExpressionSyntax node.
    /// </summary>
    ...
    class TupleExpressionSyntax : ExpressionSyntax {
        ...
        public TupleExpressionSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkTupleExpressionSyntax; }
        }
        ...
        public void addArguments(uint nodeId) {
            if (!fact.getExist(nodeId))
                throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.TupleExpressionSyntax.addArguments(NodeId)", "The end point of the edge does not exist.");
            Columbus.Csharp.Asg.Nodes.Base.Base node = fact.getRef(nodeId);
            if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkArgumentSyntax)) {
                ArgumentsContainer.AddLast(nodeId);
            } else {
                throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.TupleExpressionSyntax.setArguments(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
            }
           setParentEdge(nodeId, (uint)Types.EdgeKind.edkTupleExpressionSyntax_Arguments);
            if (fact.ReverseEdges != null)
              fact.ReverseEdges.insertEdge(nodeId, this.Id, Types.EdgeKind.edkTupleExpressionSyntax_Arguments);
        }
        ...
        public override void load(IO binIo) {
            ...
            while (_id != 0) {
              ...
              setParentEdge(_id, (uint)Types.EdgeKind.edkTupleExpressionSyntax_Arguments);
              _id = binIo.readUInt4();
            }
        }
