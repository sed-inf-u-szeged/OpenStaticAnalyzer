...
namespace Columbus.Csharp.Asg.Nodes.Expression {
    ...
    /// <summary>
    /// DeclarationExpressionSyntax class, which represents the expression::DeclarationExpressionSyntax node.
    /// </summary>
    ...
    class DeclarationExpressionSyntax : ExpressionSyntax {
        ...
        public DeclarationExpressionSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
            m_Designation = 0;
            m_Type = 0;
        }
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkDeclarationExpressionSyntax; }
        }
        ...
        public void setDesignation(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.DeclarationExpressionSyntax.setDesignation(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkVariableDesignationSyntax)) {
                    ...
                    setParentEdge(m_Designation, (uint)Types.EdgeKind.edkDeclarationExpressionSyntax_Designation);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.DeclarationExpressionSyntax.setDesignation(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Designation != 0) {
                    removeParentEdge(m_Designation);
        ...
        public void setType(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.DeclarationExpressionSyntax.setType(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkTypeSyntax)) {
                    ...
                    m_Type = nodeId;
                    setParentEdge(m_Type, (uint)Types.EdgeKind.edkDeclarationExpressionSyntax_Type);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.DeclarationExpressionSyntax.setType(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Type != 0) {
                    removeParentEdge(m_Type);
