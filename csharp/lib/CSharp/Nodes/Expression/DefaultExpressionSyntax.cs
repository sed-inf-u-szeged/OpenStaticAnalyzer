...
namespace Columbus.Csharp.Asg.Nodes.Expression {
    ...
    /// <summary>
    /// DefaultExpressionSyntax class, which represents the expression::DefaultExpressionSyntax node.
    /// </summary>
    ...
    class DefaultExpressionSyntax : ExpressionSyntax {
        ...
        public DefaultExpressionSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
            m_Type = 0;
        }
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkDefaultExpressionSyntax; }
        }
        ...
        public void setType(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.DefaultExpressionSyntax.setType(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkTypeSyntax)) {
                    ...
                    m_Type = nodeId;
                    setParentEdge(m_Type, (uint)Types.EdgeKind.edkDefaultExpressionSyntax_Type);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.DefaultExpressionSyntax.setType(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Type != 0) {
                    removeParentEdge(m_Type);
