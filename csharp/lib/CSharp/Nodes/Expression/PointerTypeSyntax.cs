...
namespace Columbus.Csharp.Asg.Nodes.Expression {
    ...
    /// <summary>
    /// PointerTypeSyntax class, which represents the expression::PointerTypeSyntax node.
    /// </summary>
    ...
    class PointerTypeSyntax : TypeSyntax {
        ...
        public PointerTypeSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
            m_ElementType = 0;
        }
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkPointerTypeSyntax; }
        }
        ...
        public void setElementType(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.PointerTypeSyntax.setElementType(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkTypeSyntax)) {
                    ...
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.PointerTypeSyntax.setElementType(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_ElementType != 0) {
                    removeParentEdge(m_ElementType);
