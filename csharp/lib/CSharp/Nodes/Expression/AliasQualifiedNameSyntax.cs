...
namespace Columbus.Csharp.Asg.Nodes.Expression {
    ...
    /// <summary>
    /// AliasQualifiedNameSyntax class, which represents the expression::AliasQualifiedNameSyntax node.
    /// </summary>
    ...
    class AliasQualifiedNameSyntax : NameSyntax {
        ...
        /// <summary>The id of the node the Alias edge points to.</summary>
        protected uint m_Alias;
        ...
        public AliasQualifiedNameSyntax(uint nodeId, Factory factory) : base(nodeId, factory) {
            m_Alias = 0;
            m_Name = 0;
        }
        ...
        public override Types.NodeKind NodeKind {
            get { return Types.NodeKind.ndkAliasQualifiedNameSyntax; }
        }
        ...
        /// <summary>
        /// Gives back the id of the node the Alias edge points to.
        /// </summary>
        /// <returns>Returns the end point of the m_Alias edge.</returns>
        /// <exception cref="Columbus.Csharp.Asg.CsharpException">Throws CsharpException if the edge id is invalid.</exception>
        public uint getAlias() {
            if (fact.getIsFiltered(m_Alias))
                return 1;
            else
                return m_Alias;
        }
        ...
        public void setAlias(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.AliasQualifiedNameSyntax.setAlias(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkIdentifierNameSyntax)) {
                    ...
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.AliasQualifiedNameSyntax.setAlias(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Alias != 0) {
                    removeParentEdge(m_Alias);
        ...
        public void setName(uint nodeId) {
            if (nodeId != 0) {
                if (!fact.getExist(nodeId))
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.AliasQualifiedNameSyntax.setName(NodeId)", "The end point of the edge does not exist.");
                ...
                if (Common.getIsBaseClassKind(node.NodeKind, Types.NodeKind.ndkSimpleNameSyntax)) {
                    ...
                    m_Name = nodeId;
                    setParentEdge(m_Name, (uint)Types.EdgeKind.edkAliasQualifiedNameSyntax_Name);
                } else {
                    throw new CsharpException("Columbus.Csharp.Asg.Nodes.Expression.AliasQualifiedNameSyntax.setName(NodeId)", "Invalid NodeKind (" + node.NodeKind.ToString() + ").");
                }
            } else {
                if (m_Name != 0) {
                    removeParentEdge(m_Name);
